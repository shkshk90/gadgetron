...
RUN mkdir -p /downloads \
    && curl --output /downloads/cuda-keyring_1.1-1_all.deb --silent --location https://developer.download.nvidia.com/compute/cuda/repos/ubuntu2404/x86_64/cuda-keyring_1.1-1_all.deb \
    && dpkg -i /downloads/cuda-keyring_1.1-1_all.deb \
    && apt-get update \
    && apt-get install -y \
