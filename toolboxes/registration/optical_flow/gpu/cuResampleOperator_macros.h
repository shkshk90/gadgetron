...
/*
DPCT1049:110: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit, query
info::device::max_work_group_size. Adjust the work-group size if needed.
*/
/*
DPCT1049:111: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit, query
info::device::max_work_group_size. Adjust the work-group size if needed.
*/
/*
DPCT1049:112: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit, query
info::device::max_work_group_size. Adjust the work-group size if needed.
*/
/*
DPCT1129:109: The type "vector_td<unsigned int, D>" is used in the SYCL kernel, but it is not device copyable. The
...
#define DECLARE_CU_RESAMPLE_OPERATOR_SUPPORT(COMPONENT)                                                                \
        ...
        if (surplus == 1) {                                                                                            \
            ...
            dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {                                                \
                ...
                auto num_batches_ct4 = num_batches;                                                                    \
                                                                                                                       \
                cgh.parallel_for<dpct_kernel_name<class mult_M_kernel_batch_1c5b70, T, dpct_kernel_scalar<D>>>(        \
                    sycl::nd_range<3>(gridDim * blockDim, blockDim), [=](sycl::nd_item<3> item_ct1) {                  \
                        mult_M_kernel_batch<T, D>(in_get_data_ptr_ct0, out_get_data_ptr_ct1,                           \
                                                  this_offsets__get_data_ptr_ct2,                                      \
            ...
            dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {                                                \
                ...
                auto extended_dim_ct5 = extended_dim;                                                                  \
                                                                                                                       \
                cgh.parallel_for<dpct_kernel_name<class mult_M_kernel_extended_f0e8b1, T, dpct_kernel_scalar<D>>>(     \
                    sycl::nd_range<3>(gridDim * blockDim, blockDim), [=](sycl::nd_item<3> item_ct1) {                  \
                        mult_M_kernel_extended<T, D>(in_get_data_ptr_ct0, out_get_data_ptr_ct1,                        \
                                                     this_offsets__get_data_ptr_ct2,                                   \
                                                     vector_td_unsigned_int_D_matrix_size_ct3,                         \
                                                     in_get_number_of_elements_ct4, extended_dim_ct5);                 \
                    });                                                                                                \
            });                                                                                                        \
        }                                                                                                              \
                                                                                                                       \
    ...
    {                                                                                                                  \
        ...
        if (surplus == 1) {                                                                                            \
            ...
            if ((in->get_number_of_elements() % num_disp_vectors) != 0) {                                              \
                throw cuda_error(                                                                                      \
                    "cuResampleOperator::mult_MH(): in/out array dimensions mismatch displacement field (1).");        \
            }                                                                                                          \
        }                                                                                                              \
                                                                                                                       \
        if (surplus == 2) {                                                                                            \
            ...
            if (in->get_number_of_dimensions() != (D + 1) || in->get_number_of_elements() != num_disp_vectors) {       \
                throw cuda_error(                                                                                      \
                    "cuResampleOperator::mult_MH(): output array dimensions mismatch displacement field.");            \
            }                                                                                                          \
        }                                                                                                              \
                                                                                                                       \
        ...
        {                                                                                                              \
            ...
            dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {                                                \
                ...
                auto num_batches_ct7 = num_batches;                                                                    \
                                                                                                                       \
                cgh.parallel_for<dpct_kernel_name<class mult_MH_kernel_c6fecd, T, dpct_kernel_scalar<D>>>(             \
                    sycl::nd_range<3>(gridDim * blockDim, blockDim), [=](sycl::nd_item<3> item_ct1) {                  \
                        mult_MH_kernel<T, D>(                                                                          \
                            in_get_data_ptr_ct0, tmp_out_get_data_ptr_ct1, raw_pointer_cast_this_weights__ct2,         \
