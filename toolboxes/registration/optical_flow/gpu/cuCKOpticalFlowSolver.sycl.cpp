...
void CorneliusKanade_kernel(const REAL*, const REAL*, const REAL*, REAL*, typename uintd<D>::Type,
                                       unsigned int, REAL, REAL, REAL, unsigned int*, char *_shared_mem);
...
boost::shared_ptr<cuNDArray<T>> cuCKOpticalFlowSolver<T, D>::core_solver(cuNDArray<T>* gradient_image,
    ...
    while (true) {
        ...
        */
        /*
        DPCT1129:1: The type "vector_td<unsigned int, D>" is used in the SYCL kernel, but it is not device copyable. The
...
template <class REAL, unsigned int D>
/*
DPCT1110:0: The total declared local variable size in device function CorneliusKanade_kernel exceeds 128 bytes and may
