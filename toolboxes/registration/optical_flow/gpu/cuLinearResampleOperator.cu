...
namespace Gadgetron{
  ...
  cuLinearResampleOperator<T,D>::write_sort_arrays( void* thrust_device_vector_unsigned_int_sort_keys )
  {
    // thrust::device_vector<unsigned int> &sort_keys
    if (thrust_device_vector_unsigned_int_sort_keys == nullptr) return;
    thrust::device_vector<unsigned int> &sort_keys = *(thrust::device_vector<unsigned int>*)thrust_device_vector_unsigned_int_sort_keys;
