...
namespace Gadgetron{
  ...
  public:
/* DPCT_ORIG 		static __device__ __inline__ void apply(T& val,const T* __restrict__ in, const typename
 * intd<D>::Type dims,const typename intd<D>::Type co, typename intd<D>::Type& stride){*/
                static __inline__ void apply(T& val, const T* __restrict__ in, const typename intd<D>::Type dims,
                                             const typename intd<D>::Type co, typename intd<D>::Type& stride) {
                        for (int d = -1; d < 2; d++)
				stride[dim]=d;
  ...
  public:
/* DPCT_ORIG   	static __device__ __inline__ void apply(T& val,const T* __restrict__ in, const typename intd<D>::Type
 * dims,const typename intd<D>::Type co, typename intd<D>::Type& stride){*/
        static __inline__ void apply(T& val, const T* __restrict__ in, const typename intd<D>::Type dims,
                                     const typename intd<D>::Type co, typename intd<D>::Type& stride) {
...
/* DPCT_ORIG   template<class REAL, class T, unsigned int D> __global__ void
  ...
  void laplace_kernel(typename intd<D>::Type dims, const T* __restrict__ in, T* __restrict__ out)
  {
...
/* DPCT_ORIG     dim3 dimBlock( dims[0] );*/
    dpct::dim3 dimBlock(dims[0]);
/* DPCT_ORIG     dim3 dimGrid( prod(dims)/dims[0] );*/
    dpct::dim3 dimGrid(prod(dims) / dims[0]);

    // Invoke kernel
/* DPCT_ORIG     laplace_kernel<typename realType<T>::Type ,T,D><<< dimGrid, dimBlock >>> (dims, in->get_data_ptr(),
 * out->get_data_ptr() );*/
    /*
    DPCT1049:89: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit, query
