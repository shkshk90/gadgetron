...
/* DPCT_ORIG     	first_order_partial_derivative_kernel<T,D><<< dimGrid, dimBlock >>>
        ...
        /*
        DPCT1049:91: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit, query
        info::device::max_work_group_size. Adjust the work-group size if needed.
        */
    /*
    DPCT1129:90: The type "vector_td<int, D>" is used in the SYCL kernel, but it is not device copyable. The
...
/* DPCT_ORIG 			second_order_partial_derivative_kernel<T,D><<< dimGrid, dimBlock >>>
        ( vector_td<int,D>(forwards_stride), vector_td<int,D>(adjoint_stride), vector_td<int,D>(dims),
          in->get_data_ptr()+i*prod(dims), out->get_data_ptr()+i*prod(dims) );*/
                        /*
                        DPCT1049:93: The work-group size passed to the SYCL kernel may exceed the limit. To get the
                        device limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
                        */
    /*
    DPCT1129:92: The type "vector_td<int, D>" is used in the SYCL kernel, but it is not device
