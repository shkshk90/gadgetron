...
using namespace Gadgetron;
  ...
  template<typename T, typename S, typename F> struct cuNDArray_applicator
  {
    __device__ T operator()(const T& t, const S& s) const {F f; return f(t, s);}
  };
  ...
  static void equals_transform(cuNDArray<T> &x, const cuNDArray<S> &y){
    if (x.dimensions_equal(y)){
      thrust::transform(x.begin(), x.end(), y.begin(), x.begin(), cuNDArray_applicator<T, S, F>{} );
    } else if (compatible_dimensions(x,y))
      {
        if (y.get_number_of_elements() < x.get_number_of_elements()) {
          ...
          thrust::permutation_iterator<thrust::device_ptr<S>, transform_it> p = thrust::make_permutation_iterator(
                  y.begin(), indices);
          thrust::transform(x.begin(), x.end(), p, x.begin(), cuNDArray_applicator<T, S, F>{});
        } else {
          thrust::transform(x.begin(),x.end(),y.begin(),x.begin(), cuNDArray_applicator<T, S, F>{});
        }
