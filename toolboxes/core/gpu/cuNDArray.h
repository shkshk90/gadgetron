...
namespace Gadgetron{
    ...
    cuNDArray<T>::cuNDArray(const cuNDArray<T> &a) : Gadgetron::NDArray<T>::NDArray() 
    {
        ...
        if (a.device_ == this->device_) {
            ...
            /*
            DPCT1093:16: The "a.device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            dpct::select_device(a.device_);
            boost::shared_ptr< hoNDArray<T> > tmp = a.to_host();
            /*
            DPCT1093:17: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            ...
            dpct::err0 err = DPCT_CHECK_ERROR(dpct::get_in_order_queue()
                                                  ...
                                                  .wait());
            /*
            DPCT1000:19: Error handling if-stmt was detected but could not be rewritten.
            */
            if (err != 0) {
                /*
                DPCT1001:18: The statement could not be removed.
                */
                deallocate_memory();
                this->data_ = 0;
    ...
    template <typename T> cuNDArray<T>& cuNDArray<T>::operator=(const cuNDArray<T>& rhs) try {
        ...
        if (dimensions_match && (rhs.device_ == cur_device) && (cur_device == this->device_)) {
            ...
            /*
            DPCT1093:20: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            ...
            if (this->device_ == rhs.device_) {
                if (DPCT_CHECK_ERROR(
                    ...
                    /*
                    DPCT1093:21: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                ...
                /*
                DPCT1093:22: The "rhs.device_" device may be not the one intended for use. Adjust the selected device if
                needed.
                */
                if (DPCT_CHECK_ERROR(dpct::select_device(rhs.device_)) != 0) {
                    /*
                    DPCT1093:23: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
                ...
                /*
                DPCT1093:24: The "this->device_" device may be not the one intended for use. Adjust the selected device
                if needed.
                */
                if (DPCT_CHECK_ERROR(dpct::select_device(this->device_)) != 0) {
                    /*
                    DPCT1093:25: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
                ...
                if (DPCT_CHECK_ERROR(dpct::get_in_order_queue()
                                         ...
                                         .wait()) != 0) {
                    /*
                    DPCT1093:26: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
            ...
            /*
            DPCT1093:27: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
    ...
    template <typename T> cuNDArray<T>& cuNDArray<T>::operator=(const hoNDArray<T>& rhs) try {
        ...
        if (dimensions_match && (cur_device == this->device_)) {
            ...
            /*
            DPCT1093:28: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(this->device_)));
            if( !dimensions_match ){
            ...
            if (DPCT_CHECK_ERROR(
                    dpct::get_in_order_queue()
                        ...
                        .wait()) != 0) {
                    /*
                    DPCT1093:29: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
            ...
            /*
            DPCT1093:30: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            if (DPCT_CHECK_ERROR(dpct::select_device(cur_device)) != 0) {
                throw cuda_error("cuNDArray::operator=: unable to restore to current device");
    ...
    template <typename T> inline void cuNDArray<T>::set_device(int device) try {
        ...
        /*
        DPCT1093:5: The "device_" device may be not the one intended for use. Adjust the selected device if needed.
        */
        ...
        deallocate_memory();
        /*
        DPCT1093:6: The "device" device may be not the one intended for use. Adjust the selected device if needed.
        */
        if (DPCT_CHECK_ERROR(dpct::select_device(device)) != 0) {
            /*
            DPCT1093:7: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            dpct::select_device(cur_device);
        ...
        if (DPCT_CHECK_ERROR(dpct::get_in_order_queue()
                                 ...
                                 .wait()) != 0) {
            /*
            DPCT1093:8: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            dpct::select_device(cur_device);
        ...
        /*
        DPCT1093:9: The "cur_device" device may be not the one intended for use. Adjust the selected device if needed.
        */
        if (DPCT_CHECK_ERROR(dpct::select_device(cur_device)) != 0) {
    ...
    template <typename T> void cuNDArray<T>::allocate_memory() try {
        ...
        if (device_ != device_no_old) {
            /*
            DPCT1093:10: The "device_" device may be not the one intended for use. Adjust the selected device if needed.
            */
        ...
        if (DPCT_CHECK_ERROR((this->data_) = (typename std::remove_reference<decltype(this->data_)>::type)
            ...
            /*
            DPCT1106:31: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
        ...
        if (device_ != device_no_old) {
            /*
            DPCT1093:11: The "device_no_old" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
    ...
    template <typename T> void cuNDArray<T>::deallocate_memory() try {
        if (this->data_) {
            ...
            if (device_ != device_no_old) {
                /*
                DPCT1093:14: The "device_" device may be not the one intended for use. Adjust the selected device if
                needed.
                */
            ...
            if (device_ != device_no_old) {
                /*
                DPCT1093:15: The "device_no_old" device may be not the one intended for use. Adjust the selected device
                if needed.
                */
