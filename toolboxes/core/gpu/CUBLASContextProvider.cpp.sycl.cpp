...
CUBLASContextProvider::~CUBLASContextProvider() try {
        ...
        while (it != handles_.end()) {
                /*
                DPCT1093:74: The "it->first" device may be not the one intended
...
dpct::blas::descriptor_ptr *
CUBLASContextProvider::getCublasHandle(int device_no) try {
	...
	if (number_of_devices == 0) {
	  ...
	  int current_device_no;
        if (DPCT_CHECK_ERROR(current_device_no =
                                 dpct::get_current_device_id()) != 0) {
	...
	if (current_device_no != device_no) {
		//We must switch context
                /*
                DPCT1093:75: The "device_no" device may be not the one intended
                for use. Adjust the selected device if needed.
                */
	...
	if (current_device_no != device_no) {
		//We must switch context back
                /*
                DPCT1093:76: The "current_device_no" device may be not the one
