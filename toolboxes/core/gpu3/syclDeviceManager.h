...
class syclDeviceManager {
  ...
  private:
    ...
    static syclDeviceManager* _instance;
};

template <class T> struct syclDataType {};

template <> struct syclDataType<float> {
    static constexpr dpct::library_data_t value = dpct::library_data_t::real_float;
};

template <> struct syclDataType<double> {
    static constexpr dpct::library_data_t value = dpct::library_data_t::real_double;
};

template <> struct syclDataType<std::complex<float>> {
    static constexpr dpct::library_data_t value = dpct::library_data_t::complex_float;
};

template <> struct syclDataType<std::complex<double>> {
    static constexpr dpct::library_data_t value = dpct::library_data_t::complex_double;
};

template <class T> constexpr dpct::library_data_t cuda_datatype() { return syclDataType<T>::value; }
} // namespace Gadgetron