...
namespace Gadgetron {
  ...
  {    
    int cur_device = syclDeviceManager::Instance()->getCurrentDevice();
    int maxGridDim = syclDeviceManager::Instance()->max_griddim(cur_device);
    int maxBlockDim = syclDeviceManager::Instance()->max_blockdim(cur_device);
    ...
    *gridDim = dim3((number_of_elements+blockDim->x-1)/blockDim->x, num_batches);
    ...
    if( gridDim->x > maxGridDim || gridDim->y > maxGridDim){
      ...
      throw std::runtime_error("setup_grid(): too many elements requested.");
    }
  }
}
