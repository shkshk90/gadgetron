...
namespace Gadgetron {
                                             ...
                                             int target_coils, sycl::int2 kernel_size, int acceleration_factor, int set)
  {
    ...
    if (idx_in < coefficients_in_set) {
      ...
      if ((coil == coilg) && (kx == 0) && (ky == 0) && (set == 0)) {
        kernel[coilg * source_coils * (kernel_size.y() * acceleration_factor) * kernel_size.x() +
               coil * (kernel_size.y() * acceleration_factor) * kernel_size.x() +
               ((kernel_size.y() >> 1) * acceleration_factor) * kernel_size.x() + (kernel_size.x() >> 1)]
            ._real = 1;
      }
    }
  }
  ...
  void scale_and_copy_unmixing_coeffs(const complext<float> * __restrict__ unmixing,
                                                 complext<float> * __restrict__ out,
                                                 int elements,
                                                 int coils,
                                                 float scale_factor)
  ...
  void conj_csm_coeffs(const complext<float> * __restrict__ csm,
                                  complext<float> * __restrict__ out,
                                  int source_elements,
                                  int target_elements)
  {
  ...
  void single_channel_coeffs(complext<float> * out,
                                        int channel_no,
    ...
    for (unsigned int set = 0; set < acceleration_factor-1; set++)
      {
        ...
        */
        ...
        /*
        DPCT1010:42: SYCL uses exceptions to report errors and does not use the error codes. The cudaGetLastError
        ...
        if (err != 0) {
          ...
          */
          std::cerr << "htgrappa_calculate_grappa_unmixing: Unable to form system matrix: " <<
              /*
              DPCT1009:43: SYCL reports errors using exceptions and does not use error codes. Please replace the
...

    {
      ...
      ht_grappa_solve_spd_system(AHA, AHrhs);
        ...
        */
        ...
        /*
        DPCT1010:44: SYCL uses exceptions to report errors and does not use the error codes. The cudaGetLastError
        function call was replaced with 0. You need to rewrite this code.
        */
        err = 0;
        /*
        ...
        if (err != 0) {
          ...
          */
          std::cerr << "htgrappa_calculate_grappa_unmixing: Failed to copy calculated coefficients to kernel: " <<
              /*
              DPCT1009:45: SYCL reports errors using exceptions and does not use error codes. Please replace the
              "get_error_string_dummy(...)" with a real error-handling function.
              */
              dpct::get_error_string_dummy(err)
                    << std::endl;
    ...
    for (unsigned int c = 0; c < target_coils; c++)
      {
        ...
        */
        ...
        /*
        DPCT1010:50: SYCL uses exceptions to report errors and does not use the error codes. The cudaGetLastError
        function call was replaced with 0. You need to rewrite this code.
        */
        dpct::err0 err = 0;
        /*
        ...
        if (err != 0) {
          ...
          */
          std::cerr << "htgrappa_calculate_grappa_unmixing: Unable to pad GRAPPA kernel: " <<
              /*
              DPCT1009:51: SYCL reports errors using exceptions and does not use error codes. Please replace the
              "get_error_string_dummy(...)" with a real error-handling function.
              */
              dpct::get_error_string_dummy(err)
                    << std::endl;
        ...
        */
        ...
        /*
        DPCT1010:52: SYCL uses exceptions to report errors and does not use the error codes. The cudaGetLastError
        function call was replaced with 0. You need to rewrite this code.
        */
        err = 0;
        /*
        ...
        */
        if (err != 0) {
          ...
          */
          std::cerr << "htgrappa_calculate_grappa_unmixing: scale and add mixing coeffs: " <<
              /*
              DPCT1009:53: SYCL reports errors using exceptions and does not use error codes. Please replace the
              "get_error_string_dummy(...)" with a real error-handling function.
              */
              dpct::get_error_string_dummy(err)
                    << std::endl;
        ...
        if (uncombined_channels) {
          ...
          if (it != (*uncombined_channels).end()) {
            ...
            /*
            DPCT1049:6: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit,
                    ...
                    dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
                        ...
                        auto out_mixing_coeff_get_data_ptr_current_uncombined_index_total_elements_source_coils_ct1 =
                            (out_mixing_coeff->get_data_ptr() +
                             current_uncombined_index * total_elements * source_coils);
