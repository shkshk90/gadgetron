...
/* DPCT_ORIG   static __global__ void
  compute_system_matrix_kernel( intd2 dims,
                                int num_coils,
                                int kernel_size,
                                float_complext *kspace,
                                float_complext *A )*/
  static void compute_system_matrix_kernel(intd2 dims, int num_coils, int kernel_size, float_complext* kspace,
                                           float_complext* A)
  {
...
/* DPCT_ORIG   static __global__ void
  write_convolution_masks_kernel( intd2 dims,
                                  ...
                                  float_complext *kspace )*/
  static void write_convolution_masks_kernel(intd2 dims, int num_coils, int kernel_size, float_complext* kernels,
                                             float_complext* kspace)
  {
...
/* DPCT_ORIG     const int idx = blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x+threadIdx.x;*/
  ...
  boost::shared_ptr<cuNDArray<float_complext>> estimate_spirit_kernels(cuNDArray<float_complext>* _kspace,
                                                                       unsigned int kernel_size) try {
...
/* DPCT_ORIG     compute_system_matrix_kernel<<< gridDim, blockDim >>>
      ( intd2(kspace.get_size(0), kspace.get_size(1)), num_coils, kernel_size,
        kspace.get_data_ptr(), A.get_data_ptr() );*/
    /*
    DPCT1049:64: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit, query
    info::device::max_work_group_size. Adjust the work-group size if needed.
    */
    dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
        intd2 intd2_kspace_get_size_kspace_get_size_ct0 = intd2(kspace.get_size(0), kspace.get_size(1));
        auto kspace_get_data_ptr_ct3 = kspace.get_data_ptr();
        auto A_get_data_ptr_ct4 = A.get_data_ptr();
...
/* DPCT_ORIG       cudaMemcpy( AHA.get_data_ptr()+i*n+i, &one, sizeof(float_complext), cudaMemcpyHostToDevice );*/
      dpct::get_in_order_queue().memcpy(AHA.get_data_ptr() + i * n + i, &one, sizeof(float_complext)).wait();
    }
    CHECK_FOR_CUDA_ERROR();
...
/* DPCT_ORIG     stat = cublasCgemm( handle, CUBLAS_OP_C, CUBLAS_OP_N,
    ...
    stat = DPCT_CHECK_ERROR(oneapi::mkl::blas::column_major::gemm(
        handle->get_queue(), oneapi::mkl::transpose::conjtrans, oneapi::mkl::transpose::nontrans, n, num_coils, m,
        dpct::get_value((sycl::float2*)&alpha, handle->get_queue()), (std::complex<float>*)A.get_data_ptr(), m,
        (std::complex<float>*)kspace.get_data_ptr(), m, dpct::get_value((sycl::float2*)&beta, handle->get_queue()),
        (std::complex<float>*)rhs.get_data_ptr(), n));
...
/* DPCT_ORIG     write_convolution_masks_kernel<<< gridDim, blockDim >>>
      ( intd2(kernel_images->get_size(0), kernel_images->get_size(1)), num_coils, kernel_size,
        rhs.get_data_ptr(), kernel_images->get_data_ptr() );*/
    /*
    DPCT1049:65: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit, query
    info::device::max_work_group_size. Adjust the work-group size if needed.
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            intd2 intd2_kernel_images_get_size_kernel_images_get_size_ct0 =
                intd2(kernel_images->get_size(0), kernel_images->get_size(1));
            auto rhs_get_data_ptr_ct3 = rhs.get_data_ptr();
            auto kernel_images_get_data_ptr_ct4 = kernel_images->get_data_ptr();
