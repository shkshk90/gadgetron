...
template <class T> void ht_grappa_solve_spd_system(cuNDArray<T>& A, cuNDArray<T>& B) {
  ...
  template <class T> void ht_grappa_solve_spd_system(hoNDArray<T> *A, hoNDArray<T> *B) {
    ...
    template <class T>
    int inverse_clib_matrix(cuNDArray<T> *A, cuNDArray<T> *b,
                            cuNDArray<T> *coeff, double lamda) try {
    // A: M*N
    // b: M*K
    ...
    dpct::blas::descriptor_ptr handle =
        *CUBLASContextProvider::instance()->getCublasHandle();
...

    {
        ...
        int stat = DPCT_CHECK_ERROR(oneapi::mkl::blas::column_major::gemm(
            handle->get_queue(), oneapi::mkl::transpose::conjtrans,
            oneapi::mkl::transpose::nontrans, N, N, M,
            dpct::get_value((sycl::float2 *)&alpha, handle->get_queue()),
            (std::complex<float> *)A->get_data_ptr(), M,
            (std::complex<float> *)A->get_data_ptr(), M,
            dpct::get_value((sycl::float2 *)&beta, handle->get_queue()),
...

    {
        ...
        int stat = DPCT_CHECK_ERROR(oneapi::mkl::blas::column_major::gemm(
            handle->get_queue(), oneapi::mkl::transpose::conjtrans,
            oneapi::mkl::transpose::nontrans, N, K, M,
            dpct::get_value((sycl::float2 *)&alpha, handle->get_queue()),
            (std::complex<float> *)A->get_data_ptr(), M,
            (std::complex<float> *)b->get_data_ptr(), M,
            dpct::get_value((sycl::float2 *)&beta, handle->get_queue()),
    ...
    if ( lamda > 0 )
    {
        ...
        if (DPCT_CHECK_ERROR(dpct::get_in_order_queue()
                                 .memcpy(pAHA, AHA.get_data_ptr(),
                                         AHA_host.get_number_of_bytes())
                                 .wait()) != 0)
        {
        ...
        // apply the regularization
        double trA =
            std::sqrt(pAHA[0].x() * pAHA[0].x() + pAHA[0].y() * pAHA[0].y());
        size_t c;
        for ( c=1; c<N; c++ )
        {
