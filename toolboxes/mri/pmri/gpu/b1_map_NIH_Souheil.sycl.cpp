...
namespace Gadgetron{
                         ...
                         char *_shared_mem)
    {
        ...
        if( ro<RO && e1<E1 && n<N )
        {
            ...
            /*
            DPCT1118:1: SYCL group functions and algorithms must be encountered in converged control flow. You may need
            to adjust the code.
            */
            /*
    ...
    void computeDH_D( cuNDArray<T>* data, cuNDArray<T>* D, cuNDArray<T>* DH_D, size_t kss )
    {
            ...
            */
                {
                        ...
                        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
                                sycl::local_accessor<char, 1> _shared_mem_acc_ct1(
                                    sycl::range<1>(numOfPixels * sizeof(T) * kss * CHA), cgh);
        ...
        else
        {
            ...
            // Invoke kernel
            /*
            DPCT1049:3: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit,
            query info::device::max_work_group_size. Adjust the work-group size if needed.
            */
                {
                        auto exp_props =
