...
/* DPCT_ORIG     if (cudaGetDevice(&device_no_old) != cudaSuccess)*/
    ...
    if (this->device_ != device_no_old &&
        ...
        /*
        DPCT1093:250: The "this->device_" device may be not the one intended for use. Adjust the selected
           device if needed.
        */
    ...
    if (this->device_ != device_no_old &&
        ...
        /*
        DPCT1093:251: The "device_no_old" device may be not the one intended for use. Adjust the selected
           device if needed.
        */
...
/* DPCT_ORIG     compute_deapodization_filter_kernel<REAL, D><<<dimGrid, dimBlock>>>(
    ...
    /*
    DPCT1049:86: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit,
     * query info::device::max_work_group_size. Adjust the work-group size if needed.
    */
    /*
    DPCT1129:84: The type "vector_td<unsigned int, D>" is used in the SYCL kernel, but it is not device copyable.
     * The sycl::is_device_copyable specialization has been added for this type. Please review the code.
    */
    /*
    DPCT1129:85: The type "vector_td<REAL, D>" is used in the SYCL kernel, but it is not device copyable. The
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            auto dynamic_cast_const_cuGriddingConvolution_complext_REAL_D_KaiserKernel_this_conv__get_get_kernel_d_ct3 =
                dynamic_cast<const cuGriddingConvolution<complext<REAL>, D, KaiserKernel>*>(this->conv_.get())
                    ->get_kernel_d();

            /*
          DPCT1050:305: The template argument of the dpct_kernel_name could not be deduced. You need to
             * update this code.
          */
            cgh.parallel_for<dpct_kernel_name<class compute_deapodization_filter_kernel_f5514e, REAL,
                ...
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), [=](sycl::nd_item<3> item_ct1) {
                    compute_deapodization_filter_kernel<REAL, D>(
                        vector_td_unsigned_int_D_this_matrix_size_os__ct0, matrix_size_os_real, filter_get_data_ptr_ct2,
