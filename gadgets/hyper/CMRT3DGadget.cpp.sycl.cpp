...
int CMRT3DGadget::process(
    GadgetContainerMessage<ISMRMRD::ImageHeader> *m1,
    GadgetContainerMessage<hoNDArray<std::complex<float>>> *m2) try {
		...
		size_t offset_out =
                ...
                if (DPCT_CHECK_ERROR(
                        dpct::get_in_order_queue()
                            .memcpy(buffer_->get_data_ptr() + offset_out,
                                    host_image->get_data_ptr() + offset_in,
                                    host_image->get_size(0) *
                                        sizeof(complext<float>))
                            .wait()) != 0) {
