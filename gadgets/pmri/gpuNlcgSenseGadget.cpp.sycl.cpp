...
#define max_number_of_gpus 10
  ...
  int gpuNlcgSenseGadget::process_config(ACE_Message_Block *mb) try {
    ...
    /*
    DPCT1093:70: The "device_number_" device may be not the one intended for
  ...
  int gpuNlcgSenseGadget::process(
      GadgetContainerMessage<ISMRMRD::ImageHeader> *m1,
      GadgetContainerMessage<GenericReconJob> *m2) try {
    ...
    if( !prepared_){
      ...
      if (DPCT_CHECK_ERROR(
              dpct::get_device(device_number_).get_device_info(deviceProp)) !=
          0) {
        GDEBUG( "\nError: unable to query device properties.\n" );
        return GADGET_FAIL;
      }

      unsigned int warp_size = deviceProp.get_max_sub_group_size();
    ...
    *device_samples *= *dcw;
    ...
    for( unsigned int frame=0; frame<frames; frame++ ){
      ...
      */
      dpct::err0 err = 0;
      /*
      DPCT1000:71: Error handling if-stmt was detected but could not be
      rewritten.
      */
      if (err != 0) {
        /*
        DPCT1009:73: SYCL reports errors using exceptions and does not use error
        codes. Please replace the "get_error_string_dummy(...)" with a real
        error-handling function.
        */
        GDEBUG("\nUnable to copy result from device to host: %s",
