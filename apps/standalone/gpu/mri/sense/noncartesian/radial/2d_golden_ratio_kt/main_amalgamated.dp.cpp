// ============================================================
// Amalgamated file for target: ktsense_radial_2d
...
// ===== Begin: apps/standalone/gpu/mri/sense/noncartesian/radial/2d_golden_ratio_kt/main.cpp =====
// Gadgetron includes
...
/* DPCT_ORIG       res[i] = ::floor(vec[i]);*/
      /*
      DPCT1064:57: Migrated floor call is used in a macro/template definition and may not be valid for all
...
/* DPCT_ORIG     cuda_error(cudaError_t errN) : std::runtime_error(cudaGetErrorString(errN)) {*/
    /*
    DPCT1009:58: SYCL reports errors using exceptions and does not use error codes. Please replace the
...
/* DPCT_ORIG     cudaError_t errorCode = cudaGetLastError();*/
    /*
    DPCT1010:61: SYCL uses exceptions to report errors and does not use the error codes. The cudaGetLastError function
...
/* DPCT_ORIG             cudaSetDevice(a.device_);*/
            /*
            DPCT1093:64: The "a.device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG             cudaSetDevice(this->device_);*/
            /*
            DPCT1093:65: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG             CUDA_CALL(cudaSetDevice(this->device_));*/
            /*
            DPCT1093:68: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:69: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
...
/* DPCT_ORIG                 if( cudaSetDevice(rhs.device_) != cudaSuccess) {*/
                /*
                DPCT1093:70: The "rhs.device_" device may be not the one intended for use. Adjust the selected device if
                needed.
                */
                if (DPCT_CHECK_ERROR(dpct::select_device(rhs.device_)) != 0) {
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:71: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
...
/* DPCT_ORIG                 if( cudaSetDevice(this->device_) != cudaSuccess) {*/
                /*
                DPCT1093:72: The "this->device_" device may be not the one intended for use. Adjust the selected device
                if needed.
                */
...
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:74: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
...
/* DPCT_ORIG             if( cudaSetDevice(cur_device) != cudaSuccess) {*/
            /*
            DPCT1093:75: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG             CUDA_CALL(cudaSetDevice(this->device_));*/
            /*
            DPCT1093:85: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(this->device_)));
            if( !dimensions_match ){
...
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:86: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
...
/* DPCT_ORIG             if( cudaSetDevice(cur_device) != cudaSuccess) {*/
            /*
            DPCT1093:87: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            if (DPCT_CHECK_ERROR(dpct::select_device(cur_device)) != 0) {
                throw cuda_error("cuNDArray::operator=: unable to restore to current device");
...
/* DPCT_ORIG         if( cur_device != device_ && cudaSetDevice(device_) != cudaSuccess) {*/
        /*
        DPCT1093:76: The "device_" device may be not the one intended for use. Adjust the selected device if needed.
        */
...
/* DPCT_ORIG         if( cudaSetDevice(device) != cudaSuccess) {*/
        /*
        DPCT1093:77: The "device" device may be not the one intended for use. Adjust the selected device if needed.
        */
        if (DPCT_CHECK_ERROR(dpct::select_device(device)) != 0) {
/* DPCT_ORIG             cudaSetDevice(cur_device);*/
            /*
            DPCT1093:78: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            dpct::select_device(cur_device);
...
/* DPCT_ORIG         if( cudaSetDevice(cur_device) != cudaSuccess) {*/
        /*
        DPCT1093:80: The "cur_device" device may be not the one intended for use. Adjust the selected device if needed.
        */
        if (DPCT_CHECK_ERROR(dpct::select_device(cur_device)) != 0) {
...
/* DPCT_ORIG             if (cudaSetDevice(device_) != cudaSuccess) {*/
            /*
            DPCT1093:81: The "device_" device may be not the one intended for use. Adjust the selected device if needed.
            */
...
/* DPCT_ORIG             cudaMemGetInfo(&free, &total);*/
            /*
            DPCT1106:88: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
...
/* DPCT_ORIG             if (cudaSetDevice(device_no_old) != cudaSuccess) {*/
            /*
            DPCT1093:82: The "device_no_old" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG                 CUDA_CALL(cudaSetDevice(device_));*/
                /*
                DPCT1093:83: The "device_" device may be not the one intended for use. Adjust the selected device if
                needed.
                */
...
/* DPCT_ORIG                 CUDA_CALL(cudaSetDevice(device_no_old));*/
                /*
                DPCT1093:84: The "device_no_old" device may be not the one intended for use. Adjust the selected device
                if needed.
                */
...
// ===== Begin: toolboxes/mri/pmri/gpu/cuNonCartesianKtSenseOperator.h =====
...
// ===== End: toolboxes/mri/pmri/gpu/cuNonCartesianKtSenseOperator.h =====
...
// ===== End: apps/standalone/gpu/mri/sense/noncartesian/radial/2d_golden_ratio_kt/main.cpp =====
...
/*
DPCT1044:90: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
 ...
 * realType<T>::Type>*/
/*
DPCT1044:91: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
...
/*
DPCT1044:92: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
template <typename T> struct cuNDA_sqrt {
/* DPCT_ORIG   __device__ T operator()(const T &x) const {return sqrt(x);}*/
  /*
  DPCT1064:93: Migrated sqrt call is used in a macro/template definition and may not be valid for all macro/template
...
/* DPCT_ORIG     return sgnX*max(absX-gamma*pow(absX,p-1), typename realType<T>::Type(0));*/
    /*
    DPCT1064:113: Migrated pow call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
    /*
    DPCT1064:114: Migrated max call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
 ...
 * realType<T>::Type,T>*/
/*
DPCT1044:115: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
 ...
 * realType<T>::Type,T>*/
/*
DPCT1044:116: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
need to remove references to typedefs from thrust::binary_function in the class definition.
*/
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:182: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class cuNDArray_permute_kernel_3ea4c6, dpct_placeholder /*Fix the type mannually*/>>(
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class expand_kernel_8a922e, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class crop_kernel_be0f65, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class pad_kernel_70e853, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class fill_border_kernel_2aeda9, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class fill_border_kernel_f5605a, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class upsample_kernel_1d659d, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class downsample_kernel_8ed994, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class sum_kernel_84561e, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG       compute_radial_trajectory_golden_ratio_2d_kernel<REAL,0><<< dimGrid, dimBlock >>>
      ...
      */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class compute_radial_trajectory_golden_ratio_2d_kernel_35b838, REAL,
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class compute_radial_trajectory_golden_ratio_2d_kernel_bc550d, REAL,
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class compute_radial_trajectory_variable_angle_2d_kernel_d5c78e, REAL>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class compute_radial_trajectory_fixed_angle_2d_kernel_e0a15b, REAL>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<
                dpct_kernel_name<class compute_radial_dcw_2d_kernel_b5b7f4, REAL,
...
using namespace Gadgetron;
  ...
  /* DPCT_ORIG   class cuNDA_modulus : public thrust::unary_function<T,T>
    {*/
  /*
  DPCT1044:119: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may
...
/* DPCT_ORIG     __device__ complext<T> operator()(const complext<T> &x, const T &y) const {return x+y;}*/
  ...
  /* DPCT_ORIG   struct cuNDA_minus : public thrust::binary_function<complext<T>, T, complext<T> >
    {*/
  /*
  DPCT1044:121: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
  need to remove references to typedefs from thrust::binary_function in the class definition.
  */
  struct cuNDA_minus {
/* DPCT_ORIG     __device__ complext<T> operator()(const complext<T> &x, const T &y) const {return x-y;}*/
...
/* DPCT_ORIG     return cublasSnrm2(hndl,n,x,inc,res);*/
    /*
    DPCT1034:124: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
...
/* DPCT_ORIG     return cublasDnrm2(hndl,n,x,inc,res);*/
    /*
    DPCT1034:125: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
...
/* DPCT_ORIG     return cublasScnrm2(hndl,n,(const cuComplex*)x,inc,res);*/
    /*
    DPCT1034:126: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_float_out res_wrapper_ct4(hndl->get_queue(), res);
...
/* DPCT_ORIG     return cublasDznrm2(hndl,n,(const cuDoubleComplex*) x,inc,res);*/
    /*
    DPCT1034:127: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_double_out res_wrapper_ct4(hndl->get_queue(), res);
 ...
 * res);*/
      /*
      DPCT1034:130: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite
      this code.
      */
      return [&]() {
 ...
 * res);*/
      /*
      DPCT1034:131: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite
      this code.
      */
      return [&]() {
      dpct::blas::wrapper_float2_out res_wrapper_ct6(hndl->get_queue(), (sycl::float2*)res);
 ...
 * (cuDoubleComplex*) res);*/
      /*
      DPCT1034:132: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite
      this code.
      */
      return [&]() {
 ...
 * (cuDoubleComplex*) res);*/
      /*
      DPCT1034:133: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite
      this code.
      */
      return [&]() {
      dpct::blas::wrapper_double2_out res_wrapper_ct6(hndl->get_queue(), (sycl::double2*)res);
...
/* DPCT_ORIG     return cublasDzasum(hndl,n,(const cuDoubleComplex*) x,incx,result);*/
    /*
    DPCT1034:137: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_double_out res_wrapper_ct4(hndl->get_queue(), result);
...
/* DPCT_ORIG     if (auto res = cudaGetDevice(&old_device); res != cudaSuccess)*/
    /*
    DPCT1000:157: Error handling if-stmt was detected but could not be rewritten.
    */
    if (auto res = DPCT_CHECK_ERROR(old_device = dpct::get_current_device_id()); res != 0)
    {
      /*
      DPCT1001:156: The statement could not be removed.
      */
      throw cuda_error(res);
    }
...
/* DPCT_ORIG       if (auto res = cudaSetDevice(device); res != cudaSuccess)*/
      ...
      */
      /*
      DPCT1093:164: The "device" device may be not the one intended for use. Adjust the selected device if needed.
      */
...
/* DPCT_ORIG       _shared_mem_per_block[device] = deviceProp.sharedMemPerBlock;*/
      /*
      DPCT1019:165: local_mem_size in SYCL is not a complete equivalent of sharedMemPerBlock in CUDA. You may need to
      adjust the code.
      */
...
/* DPCT_ORIG       _max_griddim[device] = deviceProp.maxGridSize[0];*/
      /*
      DPCT1022:166: There is no exact match between the maxGridSize and the max_nd_range size. Verify the correctness of
      the code.
      */
      _max_griddim[device] = deviceProp.get_max_nd_range_size<int*>()[0];
/* DPCT_ORIG       _major[device] = deviceProp.major;*/
      /*
      DPCT1005:167: The SYCL device version is different from CUDA Compute Compatibility. You may need to rewrite this
      code.
      */
      _major[device] = deviceProp.get_major_version();
/* DPCT_ORIG       _minor[device] = deviceProp.minor;*/
      /*
      DPCT1005:168: The SYCL device version is different from CUDA Compute Compatibility. You may need to rewrite this
      code.
      */
...
/* DPCT_ORIG     if (auto res = cudaSetDevice(old_device); res != cudaSuccess)*/
    ...
    */
    /*
    DPCT1093:169: The "old_device" device may be not the one intended for use. Adjust the selected device if needed.
    */
...
/* DPCT_ORIG     CUDA_CALL(cudaMemGetInfo(&free, &total));*/
    /*
    DPCT1106:170: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
    supported by all compilers or runtimes. You may need to adjust the code.
    */
...
/* DPCT_ORIG     CUDA_CALL(cudaMemGetInfo(&free, &total));*/
    /*
    DPCT1106:171: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
    supported by all compilers or runtimes. You may need to adjust the code.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::get_current_device().get_memory_info(free, total)));
    return total;
...
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(device));*/
    /*
    DPCT1093:172: The "device" device may be not the one intended for use. Adjust the selected device if needed.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(device)));
    size_t ret = getFreeMemory();
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(oldDevice));*/
    /*
    DPCT1093:173: The "oldDevice" device may be not the one intended for use. Adjust the selected device if needed.
    */
...
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(device));*/
    /*
    DPCT1093:174: The "device" device may be not the one intended for use. Adjust the selected device if needed.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(device)));
    size_t ret = getTotalMemory();
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(oldDevice));*/
    /*
    DPCT1093:175: The "oldDevice" device may be not the one intended for use. Adjust the selected device if needed.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(oldDevice)));
    return ret;
  }
...
/* DPCT_ORIG       cublasSetPointerMode(_handle[device], CUBLAS_POINTER_MODE_HOST);*/
      /*
      DPCT1026:176: The call to cublasSetPointerMode was removed because this functionality is redundant in SYCL.
      */
    }
...
/* DPCT_ORIG       cusparseSetPointerMode(_sparse_handle[device], CUSPARSE_POINTER_MODE_HOST);*/
      /*
      DPCT1026:177: The call to cusparseSetPointerMode was removed because this functionality is redundant in SYCL.
      */
...
/* DPCT_ORIG     if (!isfinite(weight))*/
    /*
    DPCT1064:178: Migrated isfinite call is used in a macro/template definition and may not be valid for all
    macro/template uses. Adjust the code.
    */
...
/* DPCT_ORIG     if (!isfinite(weight))*/
    /*
    DPCT1064:179: Migrated isfinite call is used in a macro/template definition and may not be valid for all
    macro/template uses. Adjust the code.
    */
    if (!sycl::isfinite((double)weight))
        return;
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class write_pairs_kernel_780a1e, REAL, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:183: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class make_conv_matrix_kernel_e73cb9, dpct_placeholder /*Fix the type mannually*/,
        ...
        */
        {
            ...
            dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
                ...
                cgh.parallel_for<dpct_kernel_name<
                    class NFFT_convolve_kernel_3bf475, T, dpct_kernel_scalar<D>,
                    dpct_kernel_scalar<
...
/* DPCT_ORIG         cudaFuncSetCacheConfig(NFFT_H_convolve_kernel<T, D, K>,
        ...
        /*
        DPCT1026:89: The call to cudaFuncSetCacheConfig was removed because SYCL currently does not support configuring
        ...
        */
        {
            ...
            dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
                ...
                cgh.parallel_for<dpct_kernel_name<
                    class NFFT_H_convolve_kernel_d54fa3, T, dpct_kernel_scalar<D>,
                    dpct_kernel_scalar<
                        K>>>(sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class wrap_image_kernel_5adb0b, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
        ...
        */
        {
            ...
            dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
                ...
                cgh.parallel_for<dpct_kernel_name<
                    class NFFT_H_atomic_convolve_kernel_60b95c, T, dpct_kernel_scalar<D>,
                    dpct_kernel_scalar<
                        K>>>(sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG     if (cudaGetDevice(&device_no_old) != cudaSuccess)*/
    ...
    if (this->device_ != device_no_old &&
        ...
        /*
        DPCT1093:180: The "this->device_" device may be not the one intended for use. Adjust the selected device if
        needed.
        */
    ...
    if (this->device_ != device_no_old &&
        ...
        /*
        DPCT1093:181: The "device_no_old" device may be not the one intended for use. Adjust the selected device if
        needed.
        */
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:184: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<dpct_kernel_name<class compute_deapodization_filter_kernel_1e301c, REAL,
                                              dpct_kernel_scalar<D>, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
// ===== Begin: toolboxes/mri/pmri/gpu/cuNonCartesianKtSenseOperator.cu =====
...
// ===== End: toolboxes/mri/pmri/gpu/cuNonCartesianKtSenseOperator.cu =====
...
/* DPCT_ORIG 	timeswitch_kernel1D<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:185: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel1D_602cba, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	timeswitch_kernel2D<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel2D_c44761, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	timeswitch_kernel3D<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel3D_d63f53, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	timeswitch_kernel<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),dimsize,batchsize,nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel_2ed967, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class rss_normalize_kernel_fd04ac, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class correlation_kernel_4b5b89, REAL, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class smooth_correlation_matrices_kernel_b18d67, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class smooth_correlation_matrices_border_kernel_642114, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG       extract_csm_kernel<REAL><<< gridDim, blockDim >>>
      ...
      */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class extract_csm_kernel_b103e4, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class set_phase_reference_kernel_ca9485, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class mult_csm_kernel_4b3fdc, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG     cudaError_t err = cudaGetLastError();*/
    /*
    DPCT1010:148: SYCL uses exceptions to report errors and does not use the error codes. The cudaGetLastError function
    call was replaced with 0. You need to rewrite this code.
    */
    dpct::err0 err = 0;
/* DPCT_ORIG     if( err != cudaSuccess ){*/
    /*
    DPCT1000:147: Error handling if-stmt was detected but could not be rewritten.
    */
    if (err != 0) {
      std::stringstream ss;
      /*
      ...
      */
      ss << "mult_csm: unable to multiply with coil sensitivities: " <<
          ...
          /*
          DPCT1009:149: SYCL reports errors using exceptions and does not use error codes. Please replace the
          "get_error_string_dummy(...)" with a real error-handling function.
          */
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class mult_csm_conj_sum_kernel_c0deac, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
