// ============================================================
// Amalgamated file for target: sense_nlcg_radial_2d
...
// ===== Begin: apps/standalone/gpu/mri/sense/noncartesian/radial/2d_golden_ratio/main_nlcg.cpp =====
// Gadgetron includes
...
/* DPCT_ORIG       res[i] = ::floor(vec[i]);*/
      /*
      DPCT1064:64: Migrated floor call is used in a macro/template definition and may not be valid for all
...
/* DPCT_ORIG     cuda_error(cudaError_t errN) : std::runtime_error(cudaGetErrorString(errN)) {*/
    /*
    DPCT1009:65: SYCL reports errors using exceptions and does not use error codes. Please replace the
...
/* DPCT_ORIG     cudaError_t errorCode = cudaGetLastError();*/
    /*
    DPCT1010:68: SYCL uses exceptions to report errors and does not use the error codes. The cudaGetLastError function
    call was replaced with 0. You need to rewrite this code.
    */
    dpct::err0 errorCode = 0;
/* DPCT_ORIG     if (errorCode != cudaSuccess) {*/
    /*
    DPCT1000:67: Error handling if-stmt was detected but could not be rewritten.
    */
    if (errorCode != 0) {
      /*
...
/* DPCT_ORIG             cudaSetDevice(a.device_);*/
            /*
            DPCT1093:71: The "a.device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG             cudaSetDevice(this->device_);*/
            /*
            DPCT1093:72: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG             CUDA_CALL(cudaSetDevice(this->device_));*/
            /*
            DPCT1093:75: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
 ...
 * !=cudaSuccess) {	    */
                if (DPCT_CHECK_ERROR(dpct::get_in_order_queue().memcpy(this->data_, rhs.data_, this->elements_ * sizeof(T))) != 0) {
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:76: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
...
/* DPCT_ORIG                 if( cudaSetDevice(rhs.device_) != cudaSuccess) {*/
                /*
                DPCT1093:77: The "rhs.device_" device may be not the one intended for use. Adjust the selected device if
                needed.
                */
...
/* DPCT_ORIG                 if( cudaSetDevice(this->device_) != cudaSuccess) {*/
                /*
                DPCT1093:79: The "this->device_" device may be not the one intended for use. Adjust the selected device
                if needed.
                */
                if (DPCT_CHECK_ERROR(dpct::select_device(this->device_)) != 0) {
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:80: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
...
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:81: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
...
/* DPCT_ORIG             if( cudaSetDevice(cur_device) != cudaSuccess) {*/
            /*
            DPCT1093:82: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG             CUDA_CALL(cudaSetDevice(this->device_));*/
            /*
            DPCT1093:92: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(this->device_)));
            if( !dimensions_match ){
...
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:93: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
...
/* DPCT_ORIG             if( cudaSetDevice(cur_device) != cudaSuccess) {*/
            /*
            DPCT1093:94: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            if (DPCT_CHECK_ERROR(dpct::select_device(cur_device)) != 0) {
                throw cuda_error("cuNDArray::operator=: unable to restore to current device");
...
/* DPCT_ORIG         if( cur_device != device_ && cudaSetDevice(device_) != cudaSuccess) {*/
        /*
        DPCT1093:83: The "device_" device may be not the one intended for use. Adjust the selected device if needed.
        */
...
/* DPCT_ORIG         if( cudaSetDevice(device) != cudaSuccess) {*/
        /*
        DPCT1093:84: The "device" device may be not the one intended for use. Adjust the selected device if needed.
        */
        if (DPCT_CHECK_ERROR(dpct::select_device(device)) != 0) {
/* DPCT_ORIG             cudaSetDevice(cur_device);*/
            /*
            DPCT1093:85: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            dpct::select_device(cur_device);
...
/* DPCT_ORIG             cudaSetDevice(cur_device);*/
            /*
            DPCT1093:86: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            dpct::select_device(cur_device);
...
/* DPCT_ORIG         if( cudaSetDevice(cur_device) != cudaSuccess) {*/
        /*
        DPCT1093:87: The "cur_device" device may be not the one intended for use. Adjust the selected device if needed.
        */
        if (DPCT_CHECK_ERROR(dpct::select_device(cur_device)) != 0) {
...
/* DPCT_ORIG             if (cudaSetDevice(device_) != cudaSuccess) {*/
            /*
            DPCT1093:88: The "device_" device may be not the one intended for use. Adjust the selected device if needed.
            */
...
/* DPCT_ORIG             if (cudaSetDevice(device_no_old) != cudaSuccess) {*/
            /*
            DPCT1093:89: The "device_no_old" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG                 CUDA_CALL(cudaSetDevice(device_));*/
                /*
                DPCT1093:90: The "device_" device may be not the one intended for use. Adjust the selected device if
                needed.
                */
...
/* DPCT_ORIG                 CUDA_CALL(cudaSetDevice(device_no_old));*/
                /*
                DPCT1093:91: The "device_no_old" device may be not the one intended for use. Adjust the selected device
                if needed.
                */
...
// ===== Begin: toolboxes/solvers/gpu/cuNlcgSolver.h =====


// ===== Begin: toolboxes/solvers/nlcgSolver.h =====
...
// ===== End: toolboxes/solvers/nlcgSolver.h =====
...
// ===== End: toolboxes/solvers/gpu/cuNlcgSolver.h =====
...
/* DPCT_ORIG 		cudaMemcpy( data->get_data_ptr()+i*samples_per_reconstruction,
...
// ===== End: apps/standalone/gpu/mri/sense/noncartesian/radial/2d_golden_ratio/main_nlcg.cpp =====
...
/*
DPCT1044:97: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
 ...
 * realType<T>::Type>*/
/*
DPCT1044:98: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
...
/*
DPCT1044:99: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
template <typename T> struct cuNDA_sqrt {
/* DPCT_ORIG   __device__ T operator()(const T &x) const {return sqrt(x);}*/
  /*
  DPCT1064:100: Migrated sqrt call is used in a macro/template definition and may not be valid for all macro/template
...
/*
DPCT1044:101: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
...
/*
DPCT1044:102: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
...
/*
DPCT1044:103: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
template <typename T> struct cuNDA_reciprocal_sqrt {
/* DPCT_ORIG   __device__ T operator()(const T &x) const {return T(1)/sqrt(x);}*/
  /*
  DPCT1064:104: Migrated sqrt call is used in a macro/template definition and may not be valid for all macro/template
  uses. Adjust the code.
  */
...
/*
DPCT1044:105: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
...
/*
DPCT1044:106: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
...
/*
DPCT1044:107: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
...
/* DPCT_ORIG     return sgnX*max(absX-gamma*pow(absX,p-1), typename realType<T>::Type(0));*/
    /*
    DPCT1064:120: Migrated pow call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
    /*
    DPCT1064:121: Migrated max call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
 ...
 * realType<T>::Type,T>*/
/*
DPCT1044:122: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
 ...
 * realType<T>::Type,T>*/
/*
DPCT1044:123: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
need to remove references to typedefs from thrust::binary_function in the class definition.
*/
...
/* DPCT_ORIG     return x/s*max(s-gamma*pow(s,p-1),typename realType<T>::Type(0));*/
    /*
    DPCT1064:124: Migrated pow call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
    /*
    DPCT1064:125: Migrated max call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:191: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class cuNDArray_permute_kernel_2220b9, dpct_placeholder /*Fix the type mannually*/>>(
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class expand_kernel_c657ad, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG          crop_kernel<T,D><<< gridDim, blockDim >>>
   ...
   int,D>(matrix_size_out), in.get_data_ptr(), out.get_data_ptr(), number_of_batches, prod(matrix_size_out) );*/
         ...
         */
    /*
    DPCT1129:6: The type "vector_td<unsigned int, D>" is used in the SYCL kernel, but it is not device copyable.
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class crop_kernel_2ca7ea, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG     pad_kernel<T,D><<< gridDim, blockDim >>>
    ...
    */
    /*
    DPCT1129:8: The type "vector_td<unsigned int, D>" is used in the SYCL kernel, but it is not device copyable. The
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class pad_kernel_e34805, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG     fill_border_kernel<T,D><<< gridDim, blockDim >>>
    ...
    */
    /*
    DPCT1129:10: The type "vector_td<unsigned int, D>" is used in the SYCL kernel, but it is not device copyable. The
    sycl::is_device_copyable specialization has been added for this type. Please review the code.
    */
    {
        auto exp_props = sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::use_root_sync};
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class fill_border_kernel_6fd596, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG     fill_border_kernel<T,D><<< gridDim, blockDim >>>
    ...
    */
    /*
    DPCT1129:12: The type "vector_td<int, D>" is used in the SYCL kernel, but it is not device copyable. The
    sycl::is_device_copyable specialization has been added for this type. Please review the code.
    */
    {
        auto exp_props = sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::use_root_sync};
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class fill_border_kernel_5f07f1, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG   template<class T, unsigned int D> __global__ void
  ...
  /*
DPCT1110:14: The total declared local variable size in device function upsample_kernel exceeds 128 bytes and may cause
...
/* DPCT_ORIG     upsample_kernel<T,D><<< gridDim, blockDim >>>
    ...
    */
    /*
    DPCT1129:15: The type "vector_td<unsigned int, D>" is used in the SYCL kernel, but it is not device copyable. The
    sycl::is_device_copyable specialization has been added for this type. Please review the code.
    */
    {
        auto exp_props = sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::use_root_sync};
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class upsample_kernel_64cf3c, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG   template<class T, unsigned int D> __global__ void
  downsample_kernel( typename intd<D>::Type matrix_size_in,
                     ...
                     T * __restrict__ image_out )*/
  /*
DPCT1110:17: The total declared local variable size in device function downsample_kernel exceeds 128 bytes and may cause
high register pressure. Consult with your hardware vendor to find the total register size available and adjust the code,
or use smaller sub-group size to avoid high register pressure.
*/
  template <class T, unsigned int D>
...
/* DPCT_ORIG     downsample_kernel<T,D><<< gridDim, blockDim >>>
    ...
    */
    /*
    DPCT1129:18: The type "vector_td<int, D>" is used in the SYCL kernel, but it is not device copyable. The
    sycl::is_device_copyable specialization has been added for this type. Please review the code.
    */
    {
        auto exp_props = sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::use_root_sync};
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class downsample_kernel_95f01d, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class sum_kernel_aab399, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG       compute_radial_trajectory_golden_ratio_2d_kernel<REAL,0><<< dimGrid, dimBlock >>>
      ...
      */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class compute_radial_trajectory_golden_ratio_2d_kernel_1cb045, REAL,
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class compute_radial_trajectory_golden_ratio_2d_kernel_23c70f, REAL,
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class compute_radial_trajectory_variable_angle_2d_kernel_45b31d, REAL>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class compute_radial_trajectory_fixed_angle_2d_kernel_50bdd3, REAL>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<
                dpct_kernel_name<class compute_radial_dcw_2d_kernel_c8d367, REAL,
...
using namespace Gadgetron;
  ...
  /* DPCT_ORIG   class cuNDA_modulus : public thrust::unary_function<T,T>
    {*/
  /*
  DPCT1044:126: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may
 ...
 * transform_it;*/
          typedef oneapi::dpl::transform_iterator<cuNDA_modulus<int>, oneapi::dpl::counting_iterator<int>, int> transform_it;
...
/* DPCT_ORIG           thrust::transform(x.begin(),x.end(),y.begin(),x.begin(),F());*/
  ...
  /* DPCT_ORIG   struct cuNDA_plus : public thrust::binary_function<complext<T>, T, complext<T> >
    {*/
  /*
  DPCT1044:127: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
  need to remove references to typedefs from thrust::binary_function in the class definition.
  */
  struct cuNDA_plus {
/* DPCT_ORIG     __device__ complext<T> operator()(const complext<T> &x, const T &y) const {return x+y;}*/
  ...
  /* DPCT_ORIG   struct cuNDA_minus : public thrust::binary_function<complext<T>, T, complext<T> >
    {*/
  /*
  DPCT1044:128: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
  need to remove references to typedefs from thrust::binary_function in the class definition.
  */
  struct cuNDA_minus {
/* DPCT_ORIG     __device__ complext<T> operator()(const complext<T> &x, const T &y) const {return x-y;}*/
...
/* DPCT_ORIG       if (auto res = cudaSetDevice(device); res != cudaSuccess)*/
      ...
      */
      /*
      DPCT1093:171: The "device" device may be not the one intended for use. Adjust the selected device if needed.
      */
...
/* DPCT_ORIG       _shared_mem_per_block[device] = deviceProp.sharedMemPerBlock;*/
      /*
      DPCT1019:172: local_mem_size in SYCL is not a complete equivalent of sharedMemPerBlock in CUDA. You may need to
      adjust the code.
      */
...
/* DPCT_ORIG       _max_griddim[device] = deviceProp.maxGridSize[0];*/
      /*
      DPCT1022:173: There is no exact match between the maxGridSize and the max_nd_range size. Verify the correctness of
      the code.
      */
      _max_griddim[device] = deviceProp.get_max_nd_range_size<int*>()[0];
/* DPCT_ORIG       _major[device] = deviceProp.major;*/
      /*
      DPCT1005:174: The SYCL device version is different from CUDA Compute Compatibility. You may need to rewrite this
      code.
      */
      _major[device] = deviceProp.get_major_version();
/* DPCT_ORIG       _minor[device] = deviceProp.minor;*/
      /*
      DPCT1005:175: The SYCL device version is different from CUDA Compute Compatibility. You may need to rewrite this
      code.
      */
...
/* DPCT_ORIG     if (auto res = cudaSetDevice(old_device); res != cudaSuccess)*/
    ...
    */
    /*
    DPCT1093:176: The "old_device" device may be not the one intended for use. Adjust the selected device if needed.
    */
...
/* DPCT_ORIG     CUDA_CALL(cudaMemGetInfo(&free, &total));*/
    /*
    DPCT1106:177: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
    supported by all compilers or runtimes. You may need to adjust the code.
    */
...
/* DPCT_ORIG     CUDA_CALL(cudaMemGetInfo(&free, &total));*/
    /*
    DPCT1106:178: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
    supported by all compilers or runtimes. You may need to adjust the code.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::get_current_device().get_memory_info(free, total)));
    return total;
...
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(device));*/
    /*
    DPCT1093:179: The "device" device may be not the one intended for use. Adjust the selected device if needed.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(device)));
    size_t ret = getFreeMemory();
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(oldDevice));*/
    /*
    DPCT1093:180: The "oldDevice" device may be not the one intended for use. Adjust the selected device if needed.
    */
...
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(device));*/
    /*
    DPCT1093:181: The "device" device may be not the one intended for use. Adjust the selected device if needed.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(device)));
    size_t ret = getTotalMemory();
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(oldDevice));*/
    /*
    DPCT1093:182: The "oldDevice" device may be not the one intended for use. Adjust the selected device if needed.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(oldDevice)));
    return ret;
  }
...
/* DPCT_ORIG       cublasSetPointerMode(_handle[device], CUBLAS_POINTER_MODE_HOST);*/
      /*
      DPCT1026:183: The call to cublasSetPointerMode was removed because this functionality is redundant in SYCL.
      */
    }
...
/* DPCT_ORIG       cusparseSetPointerMode(_sparse_handle[device], CUSPARSE_POINTER_MODE_HOST);*/
      /*
      DPCT1026:184: The call to cusparseSetPointerMode was removed because this functionality is redundant in SYCL.
      */
...
/* DPCT_ORIG     if (!isfinite(weight))*/
    /*
    DPCT1064:185: Migrated isfinite call is used in a macro/template definition and may not be valid for all
    macro/template uses. Adjust the code.
    */
...
/* DPCT_ORIG     if (!isfinite(weight))*/
    /*
    DPCT1064:186: Migrated isfinite call is used in a macro/template definition and may not be valid for all
    macro/template uses. Adjust the code.
    */
    if (!sycl::isfinite((double)weight))
        return;
...
template <class T, unsigned int D, template <class, unsigned int> class K>
/*
DPCT1110:28: The total declared local variable size in device function NFFT_H_atomic_convolve_kernel exceeds 128 bytes
and may cause high register pressure. Consult with your hardware vendor to find the total register size available and
adjust the code, or use smaller sub-group size to avoid high register pressure.
*/
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class write_pairs_kernel_16ba63, REAL, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
template <class T, unsigned int D, template <class, unsigned int> class K>
/*
DPCT1110:30: The total declared local variable size in device function NFFT_H_convolve exceeds 128 bytes and may cause
high register pressure. Consult with your hardware vendor to find the total register size available and adjust the code,
or use smaller sub-group size to avoid high register pressure.
*/
__inline__ void
...
/* DPCT_ORIG     const unsigned int frame_offset = blockIdx.y*number_of_domains;*/
    const unsigned int frame_offset = sycl::ext::oneapi::this_work_item::get_nd_item<3>().get_group(1) * number_of_domains;
...
template <class T, unsigned int D>
/*
DPCT1110:31: The total declared local variable size in device function wrap_image_kernel exceeds 128 bytes and may cause
high register pressure. Consult with your hardware vendor to find the total register size available and adjust the code,
or use smaller sub-group size to avoid high register pressure.
*/
...
/* DPCT_ORIG 	make_conv_matrix_kernel<<<dimGrid,dimBlock>>>(
        ...
        */
    /*
    DPCT1129:32: The type "vector_td<int, D>" is used in the SYCL kernel, but it is not device copyable. The
    sycl::is_device_copyable specialization has been added for this type. Please review the code.
    */
    {
        auto exp_props = sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::use_root_sync};
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:192: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class make_conv_matrix_kernel_5d0482, dpct_placeholder /*Fix the type mannually*/,
...
/* DPCT_ORIG             NFFT_convolve_kernel<T, D, K>
            ...
            /*
            DPCT1049:35: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit,
            query info::device::max_work_group_size. Adjust the work-group size if needed.
            */
        /*
        DPCT1129:34: The type "vector_td<unsigned int, D>" is used in the SYCL kernel, but it is not device
        ...
        */
        {
            ...
            dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
                ...
                cgh.parallel_for<dpct_kernel_name<
                    class NFFT_convolve_kernel_341d79, T, dpct_kernel_scalar<D>,
                    dpct_kernel_scalar<
                        K>>>(sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG         cudaFuncSetCacheConfig(NFFT_H_convolve_kernel<T, D, K>,
        ...
        /*
        DPCT1026:96: The call to cudaFuncSetCacheConfig was removed because SYCL currently does not support configuring
...
/* DPCT_ORIG             NFFT_H_convolve_kernel<T, D, K>
                ...
                this->plan_.d_kernel_);*/
            /*
            DPCT1049:37: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit,
            query info::device::max_work_group_size. Adjust the work-group size if needed.
            */
        /*
        DPCT1129:36: The type "vector_td<unsigned int, D>" is used in the SYCL kernel, but it is not device
        copyable. The sycl::is_device_copyable specialization has been added for this type. Please review the code.
        */
        {
            auto exp_props =
            ...
            dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
                ...
                cgh.parallel_for<dpct_kernel_name<
                    class NFFT_H_convolve_kernel_2ca4a9, T, dpct_kernel_scalar<D>,
                    dpct_kernel_scalar<
                        K>>>(sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG         wrap_image_kernel<T, D>
        ...
        */
    /*
    DPCT1129:38: The type "vector_td<unsigned int, D>" is used in the SYCL kernel, but it is not device copyable.
    The sycl::is_device_copyable specialization has been added for this type. Please review the code.
    */
    {
        auto exp_props = sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::use_root_sync};
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class wrap_image_kernel_574517, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG             NFFT_H_atomic_convolve_kernel<T, D, K>
                ...
                this->plan_.d_kernel_);*/
            /*
            DPCT1049:41: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit,
            query info::device::max_work_group_size. Adjust the work-group size if needed.
            */
        /*
        DPCT1129:40: The type "vector_td<unsigned int, D>" is used in the SYCL kernel, but it is not device
        copyable. The sycl::is_device_copyable specialization has been added for this type. Please review the code.
        */
        {
            auto exp_props =
            ...
            dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
                ...
                cgh.parallel_for<dpct_kernel_name<
                    class NFFT_H_atomic_convolve_kernel_a29d82, T, dpct_kernel_scalar<D>,
                    dpct_kernel_scalar<
                        K>>>(sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG     if (cudaGetDevice(&device_no_old) != cudaSuccess)*/
    ...
    if (this->device_ != device_no_old &&
        ...
        /*
        DPCT1093:187: The "this->device_" device may be not the one intended for use. Adjust the selected device if
        needed.
        */
    ...
    if (this->device_ != device_no_old &&
        ...
        /*
        DPCT1093:188: The "device_no_old" device may be not the one intended for use. Adjust the selected device if
        needed.
        */
...
/* DPCT_ORIG     compute_deapodization_filter_kernel<REAL, D><<<dimGrid, dimBlock>>>(
    ...
    */
    /*
    DPCT1129:42: The type "vector_td<unsigned int, D>" is used in the SYCL kernel, but it is not device copyable. The
    sycl::is_device_copyable specialization has been added for this type. Please review the code.
    */
    /*
    DPCT1129:43: The type "vector_td<REAL, D>" is used in the SYCL kernel, but it is not device copyable. The
    sycl::is_device_copyable specialization has been added for this type. Please review the code.
    */
    {
        auto exp_props = sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::use_root_sync};
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:193: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<dpct_kernel_name<class compute_deapodization_filter_kernel_251a51, REAL,
                                              dpct_kernel_scalar<D>, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG     	first_order_partial_derivative_kernel<T,D><<< dimGrid, dimBlock >>>
        ...
        */
    /*
    DPCT1129:45: The type "vector_td<int, D>" is used in the SYCL kernel, but it is not device copyable. The
    sycl::is_device_copyable specialization has been added for this type. Please review the code.
    */
    {
        auto exp_props = sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::use_root_sync};
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<
                dpct_kernel_name<class first_order_partial_derivative_kernel_5e5daf, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 			second_order_partial_derivative_kernel<T,D><<< dimGrid, dimBlock >>>
        ( vector_td<int,D>(forwards_stride), vector_td<int,D>(adjoint_stride), vector_td<int,D>(dims),
          in->get_data_ptr()+i*prod(dims), out->get_data_ptr()+i*prod(dims) );*/
                        ...
                        */
    /*
    DPCT1129:47: The type "vector_td<int, D>" is used in the SYCL kernel, but it is not device
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<
                dpct_kernel_name<class second_order_partial_derivative_kernel_1dfb47, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
 ...
 * realType<T>::Type><<<dimGrid,dimBlock>>>(x->get_data_ptr(),g->get_data_ptr(),elements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class filter_kernel_707686, typename realType<T>::Type>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 		return val/(1+alpha*sigma)/max(REAL(1),abs(val/(1+alpha*sigma)));*/
                /*
                DPCT1064:189: Migrated abs call is used in a macro/template definition and may not be valid for all
                macro/template uses. Adjust the code.
                */
                /*
                DPCT1064:190: Migrated max call is used in a macro/template definition and may not be valid for all
                macro/template uses. Adjust the code.
                */
 ...
 * tvGradient_kernel<<<dimGrid,dimBlock>>>(in->get_data_ptr()+i*prod(dims),out->get_data_ptr()+i*prod(dims),dims,limit_,this->weight_);*/
                /*
                DPCT1049:50: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:194: The template argument of the dpct_kernel_name could not be deduced. You need to
            update this code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class tvGradient_kernel_3a2fac, REAL, dpct_placeholder /*Fix the type mannually*/,
 ...
 * tvMagnitude_kernel<<<dimGrid,dimBlock>>>(in->get_data_ptr()+i*prod(dims),out.get_data_ptr()+i*prod(dims),dims,limit_,this->weight_);*/
                /*
                DPCT1049:51: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
                limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
                */
        auto exp_props = sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::use_root_sync};
        dpct::has_capability_or_fail(dpct::get_in_order_queue().get_device(), {sycl::aspect::fp64});
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:195: The template argument of the dpct_kernel_name could not be deduced. You need to
            update this code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class tvMagnitude_kernel_c7b975, REAL, dpct_placeholder /*Fix the type mannually*/,
                                 dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class rss_normalize_kernel_c084e7, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class correlation_kernel_5b9fc0, REAL, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class smooth_correlation_matrices_kernel_c267ca, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class smooth_correlation_matrices_border_kernel_6205eb, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	    tmp_v[j*blockDim.x+i] +=
      * corrm[(k*num_batches+j)*num_elements+idx]*data_out[k*blockDim.x+i];*/
...
/* DPCT_ORIG       extract_csm_kernel<REAL><<< gridDim, blockDim >>>
      ...
      /*
      DPCT1049:56: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit, query
      info::device::max_work_group_size. Adjust the work-group size if needed.
      */
    {
        auto exp_props = sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::use_root_sync};
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class extract_csm_kernel_7f8c37, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class set_phase_reference_kernel_c3e2f9, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	out[idx + blockIdx.y*image_elements + i*image_elements*nframes] =  _in *
    * csm[idx+i*image_elements];*/
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class mult_csm_kernel_1197b4, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class mult_csm_conj_sum_kernel_b17906, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	timeswitch_kernel1D<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:196: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel1D_8b7677, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	timeswitch_kernel2D<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),nelements);*/
        /*
        DPCT1049:61: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit, query
        info::device::max_work_group_size. Adjust the work-group size if needed.
        */
    {
        auto exp_props = sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::use_root_sync};
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:197: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel2D_adcb4b, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	timeswitch_kernel3D<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:198: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel3D_345ecb, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	timeswitch_kernel<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),dimsize,batchsize,nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:199: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel_2d138a, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
