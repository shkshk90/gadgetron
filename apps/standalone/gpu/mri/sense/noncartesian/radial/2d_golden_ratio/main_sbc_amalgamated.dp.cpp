// ============================================================
// Amalgamated file for target: sense_sbc_radial_2d
...
// ===== Begin: apps/standalone/gpu/mri/sense/noncartesian/radial/2d_golden_ratio/main_sbc.cpp =====
// Gadgetron includes
...
/* DPCT_ORIG       res[i] = ::floor(vec[i]);*/
      /*
      DPCT1064:61: Migrated floor call is used in a macro/template definition and may not be valid for all
...
/* DPCT_ORIG     cuda_error(cudaError_t errN) : std::runtime_error(cudaGetErrorString(errN)) {*/
    /*
    DPCT1009:62: SYCL reports errors using exceptions and does not use error codes. Please replace the
...
/* DPCT_ORIG     cudaError_t errorCode = cudaGetLastError();*/
    /*
    DPCT1010:65: SYCL uses exceptions to report errors and does not use the error codes. The cudaGetLastError function
...
/* DPCT_ORIG             cudaSetDevice(a.device_);*/
            /*
            DPCT1093:68: The "a.device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG             cudaSetDevice(this->device_);*/
            /*
            DPCT1093:69: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:73: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
...
/* DPCT_ORIG                 if( cudaSetDevice(rhs.device_) != cudaSuccess) {*/
                /*
                DPCT1093:74: The "rhs.device_" device may be not the one intended for use. Adjust the selected device if
                needed.
                */
                if (DPCT_CHECK_ERROR(dpct::select_device(rhs.device_)) != 0) {
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:75: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
...
/* DPCT_ORIG                 if( cudaSetDevice(this->device_) != cudaSuccess) {*/
                /*
                DPCT1093:76: The "this->device_" device may be not the one intended for use. Adjust the selected device
                if needed.
                */
                if (DPCT_CHECK_ERROR(dpct::select_device(this->device_)) != 0) {
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:77: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
...
/* DPCT_ORIG             CUDA_CALL(cudaSetDevice(this->device_));*/
            /*
            DPCT1093:89: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(this->device_)));
            if( !dimensions_match ){
...
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:90: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
...
/* DPCT_ORIG             if( cudaSetDevice(cur_device) != cudaSuccess) {*/
            /*
            DPCT1093:91: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            if (DPCT_CHECK_ERROR(dpct::select_device(cur_device)) != 0) {
                throw cuda_error("cuNDArray::operator=: unable to restore to current device");
...
/* DPCT_ORIG         if( cur_device != device_ && cudaSetDevice(device_) != cudaSuccess) {*/
        /*
        DPCT1093:80: The "device_" device may be not the one intended for use. Adjust the selected device if needed.
        */
...
/* DPCT_ORIG         if( cudaSetDevice(device) != cudaSuccess) {*/
        /*
        DPCT1093:81: The "device" device may be not the one intended for use. Adjust the selected device if needed.
        */
...
/* DPCT_ORIG             cudaSetDevice(cur_device);*/
            /*
            DPCT1093:83: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            dpct::select_device(cur_device);
...
/* DPCT_ORIG         if( cudaSetDevice(cur_device) != cudaSuccess) {*/
        /*
        DPCT1093:84: The "cur_device" device may be not the one intended for use. Adjust the selected device if needed.
        */
        if (DPCT_CHECK_ERROR(dpct::select_device(cur_device)) != 0) {
...
/* DPCT_ORIG             if (cudaSetDevice(device_) != cudaSuccess) {*/
            /*
            DPCT1093:85: The "device_" device may be not the one intended for use. Adjust the selected device if needed.
            */
...
/* DPCT_ORIG             cudaMemGetInfo(&free, &total);*/
            /*
            DPCT1106:92: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
...
/* DPCT_ORIG             if (cudaSetDevice(device_no_old) != cudaSuccess) {*/
            /*
            DPCT1093:86: The "device_no_old" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG                 CUDA_CALL(cudaSetDevice(device_));*/
                /*
                DPCT1093:87: The "device_" device may be not the one intended for use. Adjust the selected device if
                needed.
                */
...
/* DPCT_ORIG                 CUDA_CALL(cudaSetDevice(device_no_old));*/
                /*
                DPCT1093:88: The "device_no_old" device may be not the one intended for use. Adjust the selected device
                if needed.
                */
...
// ===== End: apps/standalone/gpu/mri/sense/noncartesian/radial/2d_golden_ratio/main_sbc.cpp =====
...
/*
DPCT1044:94: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
 ...
 * realType<T>::Type>*/
/*
DPCT1044:95: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
...
/*
DPCT1044:96: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
template <typename T> struct cuNDA_sqrt {
/* DPCT_ORIG   __device__ T operator()(const T &x) const {return sqrt(x);}*/
  /*
  DPCT1064:97: Migrated sqrt call is used in a macro/template definition and may not be valid for all macro/template
...
/*
DPCT1044:100: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
template <typename T> struct cuNDA_reciprocal_sqrt {
/* DPCT_ORIG   __device__ T operator()(const T &x) const {return T(1)/sqrt(x);}*/
  /*
  DPCT1064:101: Migrated sqrt call is used in a macro/template definition and may not be valid for all macro/template
  uses. Adjust the code.
  */
...
/*
DPCT1044:104: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
...
/* DPCT_ORIG     return sgnX*max(absX-gamma*pow(absX,p-1), typename realType<T>::Type(0));*/
    /*
    DPCT1064:117: Migrated pow call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
    /*
    DPCT1064:118: Migrated max call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
 ...
 * realType<T>::Type,T>*/
/*
DPCT1044:119: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
 ...
 * realType<T>::Type,T>*/
/*
DPCT1044:120: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
need to remove references to typedefs from thrust::binary_function in the class definition.
*/
...
/* DPCT_ORIG     return x/s*max(s-gamma*pow(s,p-1),typename realType<T>::Type(0));*/
    /*
    DPCT1064:121: Migrated pow call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
    /*
    DPCT1064:122: Migrated max call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:186: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class cuNDArray_permute_kernel_7bfd02, dpct_placeholder /*Fix the type mannually*/>>(
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class expand_kernel_c3a141, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class crop_kernel_f20ca8, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class pad_kernel_f2e5bc, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class fill_border_kernel_25badd, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class fill_border_kernel_399ea5, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class upsample_kernel_8f3401, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class downsample_kernel_86b6ac, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class sum_kernel_2d5dbc, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG       compute_radial_trajectory_golden_ratio_2d_kernel<REAL,0><<< dimGrid, dimBlock >>>
      ...
      */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class compute_radial_trajectory_golden_ratio_2d_kernel_2a6643, REAL,
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class compute_radial_trajectory_golden_ratio_2d_kernel_86bac5, REAL,
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class compute_radial_trajectory_variable_angle_2d_kernel_f0124d, REAL>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class compute_radial_trajectory_fixed_angle_2d_kernel_ef75b2, REAL>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<
                dpct_kernel_name<class compute_radial_dcw_2d_kernel_d0886e, REAL,
...
using namespace Gadgetron;
  ...
  /* DPCT_ORIG   class cuNDA_modulus : public thrust::unary_function<T,T>
    {*/
  /*
  DPCT1044:123: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may
...
/* DPCT_ORIG           thrust::transform(x.begin(),x.end(),y.begin(),x.begin(),F());*/
  ...
  /* DPCT_ORIG   struct cuNDA_plus : public thrust::binary_function<complext<T>, T, complext<T> >
    {*/
  /*
  DPCT1044:124: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
  need to remove references to typedefs from thrust::binary_function in the class definition.
  */
  struct cuNDA_plus {
/* DPCT_ORIG     __device__ complext<T> operator()(const complext<T> &x, const T &y) const {return x+y;}*/
  ...
  /* DPCT_ORIG   struct cuNDA_minus : public thrust::binary_function<complext<T>, T, complext<T> >
    {*/
  /*
  DPCT1044:125: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
  need to remove references to typedefs from thrust::binary_function in the class definition.
  */
  struct cuNDA_minus {
/* DPCT_ORIG     __device__ complext<T> operator()(const complext<T> &x, const T &y) const {return x-y;}*/
  ...
  /* DPCT_ORIG   struct cuNDA_multiply : public thrust::binary_function<complext<T>, T, complext<T> >
    {*/
  /*
  DPCT1044:126: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
  need to remove references to typedefs from thrust::binary_function in the class definition.
  */
...
/* DPCT_ORIG     return cublasSnrm2(hndl,n,x,inc,res);*/
    /*
    DPCT1034:128: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
...
/* DPCT_ORIG     return cublasDnrm2(hndl,n,x,inc,res);*/
    /*
    DPCT1034:129: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
...
/* DPCT_ORIG     return cublasScnrm2(hndl,n,(const cuComplex*)x,inc,res);*/
    /*
    DPCT1034:130: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_float_out res_wrapper_ct4(hndl->get_queue(), res);
 ...
 * res);*/
      /*
      DPCT1034:134: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite
      this code.
      */
      return [&]() {
 ...
 * res);*/
      /*
      DPCT1034:135: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite
      this code.
      */
      return [&]() {
      dpct::blas::wrapper_float2_out res_wrapper_ct6(hndl->get_queue(), (sycl::float2*)res);
 ...
 * (cuDoubleComplex*) res);*/
      /*
      DPCT1034:136: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite
      this code.
      */
      return [&]() {
...
/* DPCT_ORIG     return cublasSasum(hndl,n,x,incx,result);*/
    /*
    DPCT1034:138: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
...
/* DPCT_ORIG     if (auto res = cudaGetDeviceCount(&_num_devices); res != cudaSuccess)*/
    /*
    DPCT1000:159: Error handling if-stmt was detected but could not be rewritten.
    */
    if (auto res = DPCT_CHECK_ERROR(_num_devices = dpct::device_count()); res != 0)
    {
      /*
      DPCT1001:158: The statement could not be removed.
      */
      _num_devices = 0;
...
/* DPCT_ORIG     if (auto res = cudaGetDevice(&old_device); res != cudaSuccess)*/
    /*
    DPCT1000:161: Error handling if-stmt was detected but could not be rewritten.
    */
    if (auto res = DPCT_CHECK_ERROR(old_device = dpct::get_current_device_id()); res != 0)
    {
      /*
      DPCT1001:160: The statement could not be removed.
      */
      throw cuda_error(res);
    }
...
/* DPCT_ORIG       if (auto res = cudaSetDevice(device); res != cudaSuccess)*/
      /*
      DPCT1000:165: Error handling if-stmt was detected but could not be rewritten.
      */
      /*
      DPCT1093:168: The "device" device may be not the one intended for use. Adjust the selected device if needed.
      */
      if (auto res = DPCT_CHECK_ERROR(dpct::select_device(device)); res != 0)
      {
        /*
        DPCT1001:164: The statement could not be removed.
        */
        throw cuda_error(res);
      }
...
/* DPCT_ORIG       if (auto res = cudaGetDeviceProperties(&deviceProp, device); res != cudaSuccess)*/
      /*
      DPCT1000:167: Error handling if-stmt was detected but could not be rewritten.
      */
      if (auto res = DPCT_CHECK_ERROR(dpct::get_device(device).get_device_info(deviceProp)); res != 0)
      {
        /*
        DPCT1001:166: The statement could not be removed.
        */
        throw cuda_error(res);
      }
...
/* DPCT_ORIG       _shared_mem_per_block[device] = deviceProp.sharedMemPerBlock;*/
      /*
      DPCT1019:169: local_mem_size in SYCL is not a complete equivalent of sharedMemPerBlock in CUDA. You may need to
      adjust the code.
      */
...
/* DPCT_ORIG       _max_griddim[device] = deviceProp.maxGridSize[0];*/
      /*
      DPCT1022:170: There is no exact match between the maxGridSize and the max_nd_range size. Verify the correctness of
      the code.
      */
      _max_griddim[device] = deviceProp.get_max_nd_range_size<int*>()[0];
/* DPCT_ORIG       _major[device] = deviceProp.major;*/
      /*
      DPCT1005:171: The SYCL device version is different from CUDA Compute Compatibility. You may need to rewrite this
      code.
      */
      _major[device] = deviceProp.get_major_version();
/* DPCT_ORIG       _minor[device] = deviceProp.minor;*/
      /*
      DPCT1005:172: The SYCL device version is different from CUDA Compute Compatibility. You may need to rewrite this
      code.
      */
...
/* DPCT_ORIG     if (auto res = cudaSetDevice(old_device); res != cudaSuccess)*/
    /*
    DPCT1000:163: Error handling if-stmt was detected but could not be rewritten.
    */
    /*
    DPCT1093:173: The "old_device" device may be not the one intended for use. Adjust the selected device if needed.
    */
    if (auto res = DPCT_CHECK_ERROR(dpct::select_device(old_device)); res != 0)
    {
      /*
      DPCT1001:162: The statement could not be removed.
      */
      throw cuda_error(res);
    }
  }
...
/* DPCT_ORIG     CUDA_CALL(cudaMemGetInfo(&free, &total));*/
    /*
    DPCT1106:174: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
    supported by all compilers or runtimes. You may need to adjust the code.
    */
...
/* DPCT_ORIG     CUDA_CALL(cudaMemGetInfo(&free, &total));*/
    /*
    DPCT1106:175: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
    supported by all compilers or runtimes. You may need to adjust the code.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::get_current_device().get_memory_info(free, total)));
    return total;
...
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(device));*/
    /*
    DPCT1093:176: The "device" device may be not the one intended for use. Adjust the selected device if needed.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(device)));
    size_t ret = getFreeMemory();
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(oldDevice));*/
    /*
    DPCT1093:177: The "oldDevice" device may be not the one intended for use. Adjust the selected device if needed.
    */
...
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(device));*/
    /*
    DPCT1093:178: The "device" device may be not the one intended for use. Adjust the selected device if needed.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(device)));
    size_t ret = getTotalMemory();
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(oldDevice));*/
    /*
    DPCT1093:179: The "oldDevice" device may be not the one intended for use. Adjust the selected device if needed.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(oldDevice)));
    return ret;
  }
...
/* DPCT_ORIG       cublasSetPointerMode(_handle[device], CUBLAS_POINTER_MODE_HOST);*/
      /*
      DPCT1026:180: The call to cublasSetPointerMode was removed because this functionality is redundant in SYCL.
      */
    }
...
/* DPCT_ORIG       cusparseSetPointerMode(_sparse_handle[device], CUSPARSE_POINTER_MODE_HOST);*/
      /*
      DPCT1026:181: The call to cusparseSetPointerMode was removed because this functionality is redundant in SYCL.
      */
...
/* DPCT_ORIG     if (!isfinite(weight))*/
    /*
    DPCT1064:182: Migrated isfinite call is used in a macro/template definition and may not be valid for all
    macro/template uses. Adjust the code.
    */
...
/* DPCT_ORIG     if (!isfinite(weight))*/
    /*
    DPCT1064:183: Migrated isfinite call is used in a macro/template definition and may not be valid for all
    macro/template uses. Adjust the code.
    */
    if (!sycl::isfinite((double)weight))
        return;
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class write_pairs_kernel_be471a, REAL, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:187: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class make_conv_matrix_kernel_5baa40, dpct_placeholder /*Fix the type mannually*/,
        ...
        */
        {
            ...
            dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
                ...
                cgh.parallel_for<dpct_kernel_name<
                    class NFFT_convolve_kernel_ccc034, T, dpct_kernel_scalar<D>,
                    dpct_kernel_scalar<
...
/* DPCT_ORIG         cudaFuncSetCacheConfig(NFFT_H_convolve_kernel<T, D, K>,
        ...
        /*
        DPCT1026:93: The call to cudaFuncSetCacheConfig was removed because SYCL currently does not support configuring
        ...
        */
        {
            ...
            dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
                ...
                cgh.parallel_for<dpct_kernel_name<
                    class NFFT_H_convolve_kernel_8a2fdd, T, dpct_kernel_scalar<D>,
                    dpct_kernel_scalar<
                        K>>>(sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class wrap_image_kernel_f94783, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
        ...
        */
        {
            ...
            dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
                ...
                cgh.parallel_for<dpct_kernel_name<
                    class NFFT_H_atomic_convolve_kernel_fe1774, T, dpct_kernel_scalar<D>,
                    dpct_kernel_scalar<
                        K>>>(sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG     if (cudaGetDevice(&device_no_old) != cudaSuccess)*/
    ...
    if (this->device_ != device_no_old &&
        ...
        /*
        DPCT1093:184: The "this->device_" device may be not the one intended for use. Adjust the selected device if
        needed.
        */
    ...
    if (this->device_ != device_no_old &&
        ...
        /*
        DPCT1093:185: The "device_no_old" device may be not the one intended for use. Adjust the selected device if
        needed.
        */
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:188: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<dpct_kernel_name<class compute_deapodization_filter_kernel_12247f, REAL,
                                              dpct_kernel_scalar<D>, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<
                dpct_kernel_name<class first_order_partial_derivative_kernel_adedf6, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<
                dpct_kernel_name<class second_order_partial_derivative_kernel_325321, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class rss_normalize_kernel_40ced6, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class correlation_kernel_b7711e, REAL, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class smooth_correlation_matrices_kernel_469cf7, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class smooth_correlation_matrices_border_kernel_4f9c85, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG       extract_csm_kernel<REAL><<< gridDim, blockDim >>>
      ...
      */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class extract_csm_kernel_3d792a, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class set_phase_reference_kernel_87aede, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class mult_csm_kernel_e7bb2e, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG     cudaError_t err = cudaGetLastError();*/
    /*
    DPCT1010:152: SYCL uses exceptions to report errors and does not use the error codes. The cudaGetLastError function
    call was replaced with 0. You need to rewrite this code.
    */
    dpct::err0 err = 0;
/* DPCT_ORIG     if( err != cudaSuccess ){*/
    /*
    DPCT1000:151: Error handling if-stmt was detected but could not be rewritten.
    */
    if (err != 0) {
      std::stringstream ss;
      /*
      ...
      */
      ss << "mult_csm: unable to multiply with coil sensitivities: " <<
          ...
          /*
          DPCT1009:153: SYCL reports errors using exceptions and does not use error codes. Please replace the
          "get_error_string_dummy(...)" with a real error-handling function.
          */
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class mult_csm_conj_sum_kernel_8cffea, REAL>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG     cudaError_t err = cudaGetLastError();*/
    /*
    DPCT1010:156: SYCL uses exceptions to report errors and does not use the error codes. The cudaGetLastError function
    call was replaced with 0. You need to rewrite this code.
    */
    dpct::err0 err = 0;
/* DPCT_ORIG     if( err != cudaSuccess ){*/
    /*
    DPCT1000:155: Error handling if-stmt was detected but could not be rewritten.
    */
    if (err != 0) {
      std::stringstream ss;
      /*
      DPCT1001:154: The statement could not be removed.
      */
      ss << "mult_csm_conj_sum: unable to combine coils " <<
          /* DPCT_ORIG 	cudaGetErrorString(err);*/
          /*
          DPCT1009:157: SYCL reports errors using exceptions and does not use error codes. Please replace the
          "get_error_string_dummy(...)" with a real error-handling function.
          */
          dpct::get_error_string_dummy(err);
      throw cuda_error(ss.str());
...
/* DPCT_ORIG 	timeswitch_kernel1D<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:189: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel1D_329f2d, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	timeswitch_kernel2D<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:190: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel2D_c8a2ee, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	timeswitch_kernel3D<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel3D_b9e0c6, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	timeswitch_kernel<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),dimsize,batchsize,nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel_9b25f5, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
