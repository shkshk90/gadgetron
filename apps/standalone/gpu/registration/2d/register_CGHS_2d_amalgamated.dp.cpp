...
/* DPCT_ORIG     cuda_error(cudaError_t errN) : std::runtime_error(cudaGetErrorString(errN)) {*/
    /*
    DPCT1009:34: SYCL reports errors using exceptions and does not use error codes. Please replace the
...
/* DPCT_ORIG     cudaError_t errorCode = cudaGetLastError();*/
    /*
    DPCT1010:37: SYCL uses exceptions to report errors and does not use the error codes. The cudaGetLastError function
...
/* DPCT_ORIG             cudaSetDevice(a.device_);*/
            /*
            DPCT1093:40: The "a.device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG             cudaSetDevice(this->device_);*/
            /*
            DPCT1093:41: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG             CUDA_CALL(cudaSetDevice(this->device_));*/
            /*
            DPCT1093:44: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG                 if( cudaSetDevice(rhs.device_) != cudaSuccess) {*/
                /*
                DPCT1093:46: The "rhs.device_" device may be not the one intended for use. Adjust the selected device if
                needed.
                */
                if (DPCT_CHECK_ERROR(dpct::select_device(rhs.device_)) != 0) {
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:47: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
...
/* DPCT_ORIG                 if( cudaSetDevice(this->device_) != cudaSuccess) {*/
                /*
                DPCT1093:48: The "this->device_" device may be not the one intended for use. Adjust the selected device
                if needed.
                */
...
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:50: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
...
/* DPCT_ORIG             CUDA_CALL(cudaSetDevice(this->device_));*/
            /*
            DPCT1093:61: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(this->device_)));
            if( !dimensions_match ){
...
/* DPCT_ORIG         if( cur_device != device_ && cudaSetDevice(device_) != cudaSuccess) {*/
        /*
        DPCT1093:52: The "device_" device may be not the one intended for use. Adjust the selected device if needed.
        */
...
/* DPCT_ORIG             cudaSetDevice(cur_device);*/
            /*
            DPCT1093:54: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            dpct::select_device(cur_device);
...
/* DPCT_ORIG         if( cudaSetDevice(cur_device) != cudaSuccess) {*/
        /*
        DPCT1093:56: The "cur_device" device may be not the one intended for use. Adjust the selected device if needed.
        */
        if (DPCT_CHECK_ERROR(dpct::select_device(cur_device)) != 0) {
...
/* DPCT_ORIG             if (cudaSetDevice(device_) != cudaSuccess) {*/
            /*
            DPCT1093:57: The "device_" device may be not the one intended for use. Adjust the selected device if needed.
            */
...
/* DPCT_ORIG             cudaMemGetInfo(&free, &total);*/
            /*
            DPCT1106:64: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
...
/* DPCT_ORIG             if (cudaSetDevice(device_no_old) != cudaSuccess) {*/
            /*
            DPCT1093:58: The "device_no_old" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG                 CUDA_CALL(cudaSetDevice(device_));*/
                /*
                DPCT1093:59: The "device_" device may be not the one intended for use. Adjust the selected device if
                needed.
                */
...
/* DPCT_ORIG                 CUDA_CALL(cudaSetDevice(device_no_old));*/
                /*
                DPCT1093:60: The "device_no_old" device may be not the one intended for use. Adjust the selected device
                if needed.
                */
...
/* DPCT_ORIG           thrust::transform(x.begin(),x.end(),y.begin(),x.begin(),F());*/
  ...
  /* DPCT_ORIG   struct cuNDA_plus : public thrust::binary_function<complext<T>, T, complext<T> >
    {*/
  /*
  DPCT1044:66: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
...
/* DPCT_ORIG     __device__ complext<T> operator()(const complext<T> &x, const T &y) const {return x+y;}*/
  ...
  /* DPCT_ORIG   struct cuNDA_minus : public thrust::binary_function<complext<T>, T, complext<T> >
    {*/
  /*
  DPCT1044:67: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
  need to remove references to typedefs from thrust::binary_function in the class definition.
  */
  struct cuNDA_minus {
/* DPCT_ORIG     __device__ complext<T> operator()(const complext<T> &x, const T &y) const {return x-y;}*/
  ...
  /* DPCT_ORIG   struct cuNDA_multiply : public thrust::binary_function<complext<T>, T, complext<T> >
    {*/
  /*
  DPCT1044:68: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
  need to remove references to typedefs from thrust::binary_function in the class definition.
  */
...
/* DPCT_ORIG     __device__ complext<T> operator()(const complext<T> &x, const T &y) const {return x*y;}*/
  ...
  /* DPCT_ORIG   struct cuNDA_divide : public thrust::binary_function<complext<T>, T, complext<T> >
    {*/
  /*
  DPCT1044:69: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
  need to remove references to typedefs from thrust::binary_function in the class definition.
  */
  struct cuNDA_divide {
/* DPCT_ORIG     __device__ complext<T> operator()(const complext<T> &x, const T &y) const {return x/y;}*/
...
/* DPCT_ORIG   __device__ T operator()(const T &x) const {return T(1)/sqrt(x);}*/
  /*
  DPCT1064:77: Migrated sqrt call is used in a macro/template definition and may not be valid for all macro/template
  uses. Adjust the code.
  */
...
/*
DPCT1044:89: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
 ...
 * realType<T>::Type,T>*/
/*
DPCT1044:95: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may need
 ...
 * realType<T>::Type,T>*/
/*
DPCT1044:96: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::binary_function in the class definition.
*/
...
/* DPCT_ORIG     return x/s*max(s-gamma*pow(s,p-1),typename realType<T>::Type(0));*/
    /*
    DPCT1064:97: Migrated pow call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
    /*
    DPCT1064:98: Migrated max call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:147: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class cuNDArray_permute_kernel_9fa4f5, dpct_placeholder /*Fix the type mannually*/>>(
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class expand_kernel_21f4bc, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class crop_kernel_75c087, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class pad_kernel_10e28c, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class fill_border_kernel_9dac2c, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class fill_border_kernel_f4492b, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class upsample_kernel_2a1668, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class downsample_kernel_5edca0, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG     return cublasScnrm2(hndl,n,(const cuComplex*)x,inc,res);*/
    /*
    DPCT1034:101: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_float_out res_wrapper_ct4(hndl->get_queue(), res);
 ...
 * res);*/
      /*
      DPCT1034:105: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite
      this code.
      */
      return [&]() {
 ...
 * res);*/
      /*
      DPCT1034:106: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite
      this code.
      */
      return [&]() {
      dpct::blas::wrapper_float2_out res_wrapper_ct6(hndl->get_queue(), (sycl::float2*)res);
 ...
 * (cuDoubleComplex*) res);*/
      /*
      DPCT1034:107: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite
      this code.
      */
      return [&]() {
 ...
 * (cuDoubleComplex*) res);*/
      /*
      DPCT1034:108: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite
      this code.
      */
      return [&]() {
      dpct::blas::wrapper_double2_out res_wrapper_ct6(hndl->get_queue(), (sycl::double2*)res);
...
/* DPCT_ORIG     return cublasIsamax(hndl,n,x,incx,result);*/
    /*
    DPCT1034:117: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_int_to_int64_out res_wrapper_ct4(hndl->get_queue(), result);
...
/* DPCT_ORIG     return cublasIdamax(hndl,n,x,incx,result);*/
    /*
    DPCT1034:118: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_int_to_int64_out res_wrapper_ct4(hndl->get_queue(), result);
...
/* DPCT_ORIG     return cublasIcamax(hndl,n, (const cuComplex* ) x,incx,result);*/
    /*
    DPCT1034:119: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_int_to_int64_out res_wrapper_ct4(hndl->get_queue(), result);
...
/* DPCT_ORIG     return cublasIzamax(hndl,n, (const cuDoubleComplex* ) x,incx,result);*/
    /*
    DPCT1034:120: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_int_to_int64_out res_wrapper_ct4(hndl->get_queue(), result);
...
/* DPCT_ORIG     int max_griddim  = deviceProp.maxGridSize[0];*/
    /*
    DPCT1022:121: There is no exact match between the maxGridSize and the max_nd_range size. Verify the correctness of
    the code.
    */
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class spatial_grad_kernel_70f614, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class temporal_grad_kernel_9b5b40, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
        ...
        */
        {
            ...
            dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
                ...
                cgh.parallel_for<dpct_kernel_name<class HornSchunk_kernel_e00fdd, T, dpct_kernel_scalar<D>>>(
                    sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG   template<class REAL, unsigned int D> __global__ void
  ...
  /*
DPCT1110:19: The total declared local variable size in device function HornSchunk_kernel exceeds 128 bytes and may cause
high register pressure. Consult with your hardware vendor to find the total register size available and adjust the code,
or use smaller sub-group size to avoid high register pressure.
*/
  template <class REAL, unsigned int D>
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class sum_kernel_2396c6, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
\
      cgh.depends_on(dpct::get_current_device().get_in_order_queues_last_events());\
\
      cgh.parallel_for<dpct_kernel_name<class mult_M_kernel_batch_417241, T, dpct_kernel_scalar<D>>>(\
...
\
      cgh.depends_on(dpct::get_current_device().get_in_order_queues_last_events());\
\
      cgh.parallel_for<dpct_kernel_name<class mult_M_kernel_extended_7abb79, T, dpct_kernel_scalar<D>>>(\
        sycl::nd_range<3>(gridDim * blockDim, blockDim), \
exp_props,         [=](sycl::nd_item<3> item_ct1) {\
...
\
      cgh.depends_on(dpct::get_current_device().get_in_order_queues_last_events());\
\
      cgh.parallel_for<dpct_kernel_name<class mult_MH_kernel_ea44b6, T, dpct_kernel_scalar<D>>>(\
        sycl::nd_range<3>(gridDim * blockDim, blockDim), \
exp_props,         [=](sycl::nd_item<3> item_ct1) {\
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class write_sort_arrays_kernel_41b13c, typename realType<T>::Type,
...
/* DPCT_ORIG     	first_order_partial_derivative_kernel<T,D><<< dimGrid, dimBlock >>>
        ...
        */
    /*
    DPCT1129:30: The type "vector_td<int, D>" is used in the SYCL kernel, but it is not device copyable. The
    sycl::is_device_copyable specialization has been added for this type. Please review the code.
    */
    {
        auto exp_props = sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::use_root_sync};
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<
                dpct_kernel_name<class first_order_partial_derivative_kernel_5bba14, T, dpct_kernel_scalar<D>>>(
...
/* DPCT_ORIG 			second_order_partial_derivative_kernel<T,D><<< dimGrid, dimBlock >>>
        ( vector_td<int,D>(forwards_stride), vector_td<int,D>(adjoint_stride), vector_td<int,D>(dims),
          in->get_data_ptr()+i*prod(dims), out->get_data_ptr()+i*prod(dims) );*/
                        /*
                        DPCT1049:33: The work-group size passed to the SYCL kernel may exceed the limit. To get the
                        device limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
                        */
    /*
    DPCT1129:32: The type "vector_td<int, D>" is used in the SYCL kernel, but it is not device
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<
                dpct_kernel_name<class second_order_partial_derivative_kernel_d36e0a, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG       if (auto res = cudaSetDevice(device); res != cudaSuccess)*/
      ...
      */
      /*
      DPCT1093:133: The "device" device may be not the one intended for use. Adjust the selected device if needed.
      */
...
/* DPCT_ORIG       if (auto res = cudaGetDeviceProperties(&deviceProp, device); res != cudaSuccess)*/
      /*
      DPCT1000:132: Error handling if-stmt was detected but could not be rewritten.
      */
...
/* DPCT_ORIG       _shared_mem_per_block[device] = deviceProp.sharedMemPerBlock;*/
      /*
      DPCT1019:134: local_mem_size in SYCL is not a complete equivalent of sharedMemPerBlock in CUDA. You may need to
      adjust the code.
      */
...
/* DPCT_ORIG       _max_griddim[device] = deviceProp.maxGridSize[0];*/
      /*
      DPCT1022:135: There is no exact match between the maxGridSize and the max_nd_range size. Verify the correctness of
      the code.
      */
      _max_griddim[device] = deviceProp.get_max_nd_range_size<int*>()[0];
/* DPCT_ORIG       _major[device] = deviceProp.major;*/
      /*
      DPCT1005:136: The SYCL device version is different from CUDA Compute Compatibility. You may need to rewrite this
      code.
      */
      _major[device] = deviceProp.get_major_version();
/* DPCT_ORIG       _minor[device] = deviceProp.minor;*/
      /*
      DPCT1005:137: The SYCL device version is different from CUDA Compute Compatibility. You may need to rewrite this
      code.
      */
...
/* DPCT_ORIG     if (auto res = cudaSetDevice(old_device); res != cudaSuccess)*/
    ...
    */
    /*
    DPCT1093:138: The "old_device" device may be not the one intended for use. Adjust the selected device if needed.
    */
...
/* DPCT_ORIG     CUDA_CALL(cudaMemGetInfo(&free, &total));*/
    /*
    DPCT1106:139: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
    supported by all compilers or runtimes. You may need to adjust the code.
    */
...
/* DPCT_ORIG     CUDA_CALL(cudaMemGetInfo(&free, &total));*/
    /*
    DPCT1106:140: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
    supported by all compilers or runtimes. You may need to adjust the code.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::get_current_device().get_memory_info(free, total)));
    return total;
...
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(device));*/
    /*
    DPCT1093:141: The "device" device may be not the one intended for use. Adjust the selected device if needed.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(device)));
    size_t ret = getFreeMemory();
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(oldDevice));*/
    /*
    DPCT1093:142: The "oldDevice" device may be not the one intended for use. Adjust the selected device if needed.
    */
...
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(device));*/
    /*
    DPCT1093:143: The "device" device may be not the one intended for use. Adjust the selected device if needed.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(device)));
    size_t ret = getTotalMemory();
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(oldDevice));*/
    /*
    DPCT1093:144: The "oldDevice" device may be not the one intended for use. Adjust the selected device if needed.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(oldDevice)));
    return ret;
  }
...
/* DPCT_ORIG       cublasSetPointerMode(_handle[device], CUBLAS_POINTER_MODE_HOST);*/
      /*
      DPCT1026:145: The call to cublasSetPointerMode was removed because this functionality is redundant in SYCL.
      */
    }
...
/* DPCT_ORIG       cusparseSetPointerMode(_sparse_handle[device], CUSPARSE_POINTER_MODE_HOST);*/
      /*
      DPCT1026:146: The call to cusparseSetPointerMode was removed because this functionality is redundant in SYCL.
      */
