// ============================================================
// Amalgamated file for target: blur_3d
...
// ===== Begin: apps/standalone/gpu/deblurring/3d/blur_3d.cpp =====
/*
...
/* DPCT_ORIG     cuda_error(cudaError_t errN) : std::runtime_error(cudaGetErrorString(errN)) {*/
    /*
    DPCT1009:23: SYCL reports errors using exceptions and does not use error codes. Please replace the
...
/* DPCT_ORIG     cudaError_t errorCode = cudaGetLastError();*/
    /*
    DPCT1010:26: SYCL uses exceptions to report errors and does not use the error codes. The cudaGetLastError function
...
/* DPCT_ORIG             cudaSetDevice(a.device_);*/
            /*
            DPCT1093:46: The "a.device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG             cudaSetDevice(this->device_);*/
            /*
            DPCT1093:47: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG             CUDA_CALL(cudaSetDevice(this->device_));*/
            /*
            DPCT1093:29: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:30: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
...
/* DPCT_ORIG                 if( cudaSetDevice(rhs.device_) != cudaSuccess) {*/
                /*
                DPCT1093:31: The "rhs.device_" device may be not the one intended for use. Adjust the selected device if
                needed.
                */
                if (DPCT_CHECK_ERROR(dpct::select_device(rhs.device_)) != 0) {
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:32: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
...
/* DPCT_ORIG                 if( cudaSetDevice(this->device_) != cudaSuccess) {*/
                /*
                DPCT1093:33: The "this->device_" device may be not the one intended for use. Adjust the selected device
                if needed.
                */
                if (DPCT_CHECK_ERROR(dpct::select_device(this->device_)) != 0) {
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:34: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
...
/* DPCT_ORIG                     cudaSetDevice(cur_device);*/
                    /*
                    DPCT1093:35: The "cur_device" device may be not the one intended for use. Adjust the selected device
                    if needed.
                    */
                    dpct::select_device(cur_device);
...
/* DPCT_ORIG             if( cudaSetDevice(cur_device) != cudaSuccess) {*/
            /*
            DPCT1093:36: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG             CUDA_CALL(cudaSetDevice(this->device_));*/
            /*
            DPCT1093:50: The "this->device_" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(this->device_)));
            if( !dimensions_match ){
...
/* DPCT_ORIG             if( cudaSetDevice(cur_device) != cudaSuccess) {*/
            /*
            DPCT1093:52: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            if (DPCT_CHECK_ERROR(dpct::select_device(cur_device)) != 0) {
                throw cuda_error("cuNDArray::operator=: unable to restore to current device");
...
/* DPCT_ORIG         if( cur_device != device_ && cudaSetDevice(device_) != cudaSuccess) {*/
        /*
        DPCT1093:37: The "device_" device may be not the one intended for use. Adjust the selected device if needed.
        */
...
/* DPCT_ORIG         if( cudaSetDevice(device) != cudaSuccess) {*/
        /*
        DPCT1093:38: The "device" device may be not the one intended for use. Adjust the selected device if needed.
        */
        if (DPCT_CHECK_ERROR(dpct::select_device(device)) != 0) {
/* DPCT_ORIG             cudaSetDevice(cur_device);*/
            /*
            DPCT1093:39: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            dpct::select_device(cur_device);
...
/* DPCT_ORIG             cudaSetDevice(cur_device);*/
            /*
            DPCT1093:40: The "cur_device" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
            dpct::select_device(cur_device);
...
/* DPCT_ORIG         if( cudaSetDevice(cur_device) != cudaSuccess) {*/
        /*
        DPCT1093:41: The "cur_device" device may be not the one intended for use. Adjust the selected device if needed.
        */
        if (DPCT_CHECK_ERROR(dpct::select_device(cur_device)) != 0) {
...
/* DPCT_ORIG             if (cudaSetDevice(device_) != cudaSuccess) {*/
            /*
            DPCT1093:42: The "device_" device may be not the one intended for use. Adjust the selected device if needed.
            */
...
/* DPCT_ORIG             cudaMemGetInfo(&free, &total);*/
            /*
            DPCT1106:53: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
...
/* DPCT_ORIG             if (cudaSetDevice(device_no_old) != cudaSuccess) {*/
            /*
            DPCT1093:43: The "device_no_old" device may be not the one intended for use. Adjust the selected device if
            needed.
            */
...
/* DPCT_ORIG                 CUDA_CALL(cudaSetDevice(device_));*/
                /*
                DPCT1093:44: The "device_" device may be not the one intended for use. Adjust the selected device if
                needed.
                */
...
/* DPCT_ORIG                 CUDA_CALL(cudaSetDevice(device_no_old));*/
                /*
                DPCT1093:45: The "device_no_old" device may be not the one intended for use. Adjust the selected device
                if needed.
                */
...
// ===== End: apps/standalone/gpu/deblurring/3d/blur_3d.cpp =====
...
/* DPCT_ORIG     return cublasSnrm2(hndl,n,x,inc,res);*/
    /*
    DPCT1034:54: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
...
/* DPCT_ORIG     return cublasDnrm2(hndl,n,x,inc,res);*/
    /*
    DPCT1034:55: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
...
/* DPCT_ORIG     return cublasScnrm2(hndl,n,(const cuComplex*)x,inc,res);*/
    /*
    DPCT1034:56: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_float_out res_wrapper_ct4(hndl->get_queue(), res);
...
/* DPCT_ORIG     return cublasDznrm2(hndl,n,(const cuDoubleComplex*) x,inc,res);*/
    /*
    DPCT1034:57: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_double_out res_wrapper_ct4(hndl->get_queue(), res);
...
/* DPCT_ORIG     return cublasSdot( hndl, n, x, incx, y, incy, res);*/
    /*
    DPCT1034:58: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
...
/* DPCT_ORIG     return cublasDdot( hndl, n, x, incx, y, incy, res);*/
    /*
    DPCT1034:59: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
 ...
 * res);*/
      /*
      DPCT1034:60: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
      code.
      */
      return [&]() {
 ...
 * res);*/
      /*
      DPCT1034:61: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
      code.
      */
      return [&]() {
      dpct::blas::wrapper_float2_out res_wrapper_ct6(hndl->get_queue(), (sycl::float2*)res);
 ...
 * (cuDoubleComplex*) res);*/
      /*
      DPCT1034:62: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
      code.
      */
      return [&]() {
 ...
 * (cuDoubleComplex*) res);*/
      /*
      DPCT1034:63: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
      code.
      */
      return [&]() {
      dpct::blas::wrapper_double2_out res_wrapper_ct6(hndl->get_queue(), (sycl::double2*)res);
...
/* DPCT_ORIG     return cublasSasum(hndl,n,x,incx,result);*/
    /*
    DPCT1034:64: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
...
/* DPCT_ORIG     return cublasDasum(hndl,n,x,incx,result);*/
    /*
    DPCT1034:65: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
...
/* DPCT_ORIG     return cublasScasum(hndl,n,(const cuComplex*) x,incx,result);*/
    /*
    DPCT1034:66: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_float_out res_wrapper_ct4(hndl->get_queue(), result);
...
/* DPCT_ORIG     return cublasDzasum(hndl,n,(const cuDoubleComplex*) x,incx,result);*/
    /*
    DPCT1034:67: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_double_out res_wrapper_ct4(hndl->get_queue(), result);
...
/* DPCT_ORIG     return cublasIsamin(hndl,n,x,incx,result);*/
    /*
    DPCT1034:68: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
...
/* DPCT_ORIG     return cublasIdamin(hndl,n,x,incx,result);*/
    /*
    DPCT1034:69: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_int_to_int64_out res_wrapper_ct4(hndl->get_queue(), result);
...
/* DPCT_ORIG     return cublasIcamin(hndl,n, (const cuComplex* ) x,incx,result);*/
    /*
    DPCT1034:70: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_int_to_int64_out res_wrapper_ct4(hndl->get_queue(), result);
...
/* DPCT_ORIG     return cublasIzamin(hndl,n, (const cuDoubleComplex* ) x,incx,result);*/
    /*
    DPCT1034:71: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_int_to_int64_out res_wrapper_ct4(hndl->get_queue(), result);
...
/* DPCT_ORIG     return cublasIsamax(hndl,n,x,incx,result);*/
    /*
    DPCT1034:72: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_int_to_int64_out res_wrapper_ct4(hndl->get_queue(), result);
...
/* DPCT_ORIG     return cublasIdamax(hndl,n,x,incx,result);*/
    /*
    DPCT1034:73: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_int_to_int64_out res_wrapper_ct4(hndl->get_queue(), result);
...
/* DPCT_ORIG     return cublasIcamax(hndl,n, (const cuComplex* ) x,incx,result);*/
    /*
    DPCT1034:74: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_int_to_int64_out res_wrapper_ct4(hndl->get_queue(), result);
...
/* DPCT_ORIG     return cublasIzamax(hndl,n, (const cuDoubleComplex* ) x,incx,result);*/
    /*
    DPCT1034:75: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this
    code.
    */
    return [&]() {
    dpct::blas::wrapper_int_to_int64_out res_wrapper_ct4(hndl->get_queue(), result);
...
/* DPCT_ORIG   __device__ T operator()(const T &x) const {return sqrt(x);}*/
  /*
  DPCT1064:79: Migrated sqrt call is used in a macro/template definition and may not be valid for all macro/template
...
/* DPCT_ORIG   __device__ T operator()(const T &x) const {return T(1)/sqrt(x);}*/
  /*
  DPCT1064:83: Migrated sqrt call is used in a macro/template definition and may not be valid for all macro/template
  uses. Adjust the code.
  */
...
/*
DPCT1044:93: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may need
to remove references to typedefs from thrust::unary_function in the class definition.
*/
...
/* DPCT_ORIG     return sgnX*max(absX-gamma*pow(absX,p-1), typename realType<T>::Type(0));*/
    /*
    DPCT1064:99: Migrated pow call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
    /*
    DPCT1064:100: Migrated max call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
 ...
 * realType<T>::Type,T>*/
/*
DPCT1044:101: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
 ...
 * realType<T>::Type,T>*/
/*
DPCT1044:102: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
need to remove references to typedefs from thrust::binary_function in the class definition.
*/
...
/* DPCT_ORIG     return x/s*max(s-gamma*pow(s,p-1),typename realType<T>::Type(0));*/
    /*
    DPCT1064:103: Migrated pow call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
    /*
    DPCT1064:104: Migrated max call is used in a macro/template definition and may not be valid for all macro/template
    uses. Adjust the code.
    */
...
using namespace Gadgetron;
  ...
  /* DPCT_ORIG   class cuNDA_modulus : public thrust::unary_function<T,T>
    {*/
  /*
  DPCT1044:105: thrust::unary_function was removed because std::unary_function has been deprecated in C++11. You may
...
/* DPCT_ORIG     __device__ complext<T> operator()(const complext<T> &x, const T &y) const {return x-y;}*/
  ...
  /* DPCT_ORIG   struct cuNDA_multiply : public thrust::binary_function<complext<T>, T, complext<T> >
    {*/
  /*
  DPCT1044:108: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
  need to remove references to typedefs from thrust::binary_function in the class definition.
  */
...
/* DPCT_ORIG     __device__ complext<T> operator()(const complext<T> &x, const T &y) const {return x*y;}*/
  ...
  /* DPCT_ORIG   struct cuNDA_divide : public thrust::binary_function<complext<T>, T, complext<T> >
    {*/
  /*
  DPCT1044:109: thrust::binary_function was removed because std::binary_function has been deprecated in C++11. You may
  need to remove references to typedefs from thrust::binary_function in the class definition.
  */
  struct cuNDA_divide {
/* DPCT_ORIG     __device__ complext<T> operator()(const complext<T> &x, const T &y) const {return x/y;}*/
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class sum_kernel_cd19c2, T>>(
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:134: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class cuNDArray_permute_kernel_3ebdbf, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class expand_kernel_af337c, T>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class crop_kernel_131c6c, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class pad_kernel_81886e, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class fill_border_kernel_1ee708, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class fill_border_kernel_f5e225, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class upsample_kernel_f90ab9, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
    ...
    */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<dpct_kernel_name<class downsample_kernel_5155da, T, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	timeswitch_kernel1D<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:135: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel1D_3d7557, dpct_placeholder /*Fix the type mannually*/>>(
...
/* DPCT_ORIG 	timeswitch_kernel2D<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel2D_f13c34, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	timeswitch_kernel3D<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            /*
            DPCT1050:137: The template argument of the dpct_kernel_name could not be deduced. You need to update this
            code.
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel3D_f0d8bc, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG 	timeswitch_kernel<<<dimGrid,dimBlock>>>(inout->get_data_ptr(),dimsize,batchsize,nelements);*/
        ...
        */
    {
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            */
            cgh.parallel_for<
                dpct_kernel_name<class timeswitch_kernel_c4d48c, dpct_placeholder /*Fix the type mannually*/>>(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG     origin_mirror_kernel<complext<REAL>,D><<< gridDim, blockDim >>>
    ...
    */
    /*
    DPCT1129:21: The type "vector_td<unsigned int, D>" is used in the SYCL kernel, but it is not device copyable. The
    sycl::is_device_copyable specialization has been added for this type. Please review the code.
    */
    {
        auto exp_props = sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::use_root_sync};
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<
                dpct_kernel_name<class origin_mirror_kernel_9b1a02, complext<REAL>, dpct_kernel_scalar<D>>>(
                sycl::nd_range<3>(gridDim * blockDim, blockDim), exp_props, [=](sycl::nd_item<3> item_ct1) {
...
/* DPCT_ORIG     if (auto res = cudaGetDeviceCount(&_num_devices); res != cudaSuccess)*/
    /*
    DPCT1000:111: Error handling if-stmt was detected but could not be rewritten.
    */
...
/* DPCT_ORIG       if (auto res = cudaSetDevice(device); res != cudaSuccess)*/
      ...
      */
      /*
      DPCT1093:120: The "device" device may be not the one intended for use. Adjust the selected device if needed.
      */
...
/* DPCT_ORIG       _shared_mem_per_block[device] = deviceProp.sharedMemPerBlock;*/
      /*
      DPCT1019:121: local_mem_size in SYCL is not a complete equivalent of sharedMemPerBlock in CUDA. You may need to
      adjust the code.
      */
...
/* DPCT_ORIG       _max_griddim[device] = deviceProp.maxGridSize[0];*/
      /*
      DPCT1022:122: There is no exact match between the maxGridSize and the max_nd_range size. Verify the correctness of
      the code.
      */
      _max_griddim[device] = deviceProp.get_max_nd_range_size<int*>()[0];
/* DPCT_ORIG       _major[device] = deviceProp.major;*/
      /*
      DPCT1005:123: The SYCL device version is different from CUDA Compute Compatibility. You may need to rewrite this
      code.
      */
      _major[device] = deviceProp.get_major_version();
/* DPCT_ORIG       _minor[device] = deviceProp.minor;*/
      /*
      DPCT1005:124: The SYCL device version is different from CUDA Compute Compatibility. You may need to rewrite this
      code.
      */
...
/* DPCT_ORIG     if (auto res = cudaSetDevice(old_device); res != cudaSuccess)*/
    ...
    */
    /*
    DPCT1093:125: The "old_device" device may be not the one intended for use. Adjust the selected device if needed.
    */
...
/* DPCT_ORIG     CUDA_CALL(cudaMemGetInfo(&free, &total));*/
    /*
    DPCT1106:126: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
    supported by all compilers or runtimes. You may need to adjust the code.
    */
...
/* DPCT_ORIG     CUDA_CALL(cudaMemGetInfo(&free, &total));*/
    /*
    DPCT1106:127: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be
    supported by all compilers or runtimes. You may need to adjust the code.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::get_current_device().get_memory_info(free, total)));
    return total;
...
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(device));*/
    /*
    DPCT1093:128: The "device" device may be not the one intended for use. Adjust the selected device if needed.
    */
    CUDA_CALL(DPCT_CHECK_ERROR(dpct::select_device(device)));
    size_t ret = getFreeMemory();
/* DPCT_ORIG     CUDA_CALL(cudaSetDevice(oldDevice));*/
    /*
    DPCT1093:129: The "oldDevice" device may be not the one intended for use. Adjust the selected device if needed.
    */
...
/* DPCT_ORIG       cublasSetPointerMode(_handle[device], CUBLAS_POINTER_MODE_HOST);*/
      /*
      DPCT1026:132: The call to cublasSetPointerMode was removed because this functionality is redundant in SYCL.
      */
    }
...
/* DPCT_ORIG       cusparseSetPointerMode(_sparse_handle[device], CUSPARSE_POINTER_MODE_HOST);*/
      /*
      DPCT1026:133: The call to cusparseSetPointerMode was removed because this functionality is redundant in SYCL.
      */
