...
TYPED_TEST(cuNDArray_elemwise_TestReal,sgnTest){
  ...
  CUDA_CALL(DPCT_CHECK_ERROR(
      dpct::get_in_order_queue()
          .memcpy(&this->Array.get_data_ptr()[91], &tmp, sizeof(TypeParam))
          .wait()));
  CUDA_CALL(DPCT_CHECK_ERROR(
      dpct::get_in_order_queue()
          .memcpy(&this->Array.get_data_ptr()[19100], &tmp2, sizeof(TypeParam))
          .wait()));
  ...
  CUDA_CALL(DPCT_CHECK_ERROR(
      dpct::get_in_order_queue()
          .memcpy(&this->Array.get_data_ptr()[9100], &tmp, sizeof(TypeParam))
          .wait()));
  CUDA_CALL(DPCT_CHECK_ERROR(
      dpct::get_in_order_queue()
          .memcpy(&this->Array.get_data_ptr()[19100], &tmp2, sizeof(TypeParam))
...
TYPED_TEST(cuNDArray_elemwise_TestReal,clampTest){
  ...
  CUDA_CALL(DPCT_CHECK_ERROR(
      dpct::get_in_order_queue()
          .memcpy(&this->Array.get_data_ptr()[354222], &tmp, sizeof(TypeParam))
          .wait()));
