...
TYPED_TEST(cuNDArray_utils_TestReal,permuteTest){
  ...
  TypeParam tmp(2);
/* DPCT_ORIG   CUDA_CALL(cudaMemcpy(&this->Array.get_data_ptr()[37], &tmp, sizeof(TypeParam),
 * cudaMemcpyHostToDevice));*/
  CUDA_CALL(DPCT_CHECK_ERROR(
      dpct::get_in_order_queue().memcpy(&this->Array.get_data_ptr()[37], &tmp, sizeof(TypeParam)).wait()));
